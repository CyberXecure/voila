...
def make_review_questions(lesson: dict) -> list[str]:
    ...
    return [
        "Care este ideea principală a acestei lecții?",
        "Ce pagini sursă susțin această lecție?",
    ]
...
def write_clean_course(outline: dict, output_path: Path) -> list[dict]:
    ...
    lines.append("")
    lines.append("Generat de Voila! v0.1.2")
    lines.append("")
    lines.append("Mod: limba sursă, fără traducere, fără generare AI")
    lines.append("Stare text: text OCR curățat pentru citire; sursa originală este păstrată în pages.md/pages.json")
    lines.append(f"Sursă: `{outline.get('source_file')}`")
    lines.append("")
    ...
    for lesson in outline.get("lessons", []):
        ...
        lines.append("")
        lines.append(f"Pagini PDF sursă: {', '.join(map(str, pages))}")
        lines.append(f"Număr de cuvinte: {lesson.get('word_count')}")
        lines.append("")
        ...
        lines.append("### Întrebări de recapitulare")
        lines.append("")
        for index, question in enumerate(questions, start=1):
            lines.append(f"{index}. {question}")
        lines.append("")

        lines.append("### Text sursă curățat")
        lines.append("")
        lines.append("```text")
