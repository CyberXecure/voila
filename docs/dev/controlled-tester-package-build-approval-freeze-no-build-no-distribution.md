...
Scope: owner-controlled build approval freeze only
Build approval: explicitly frozen and blocked
Build: explicitly blocked
Distribution: explicitly blocked

## Purpose

This milestone freezes build approval after the v0.6.17 prebuild validation dry-run.

It records that the prebuild validation chain has completed while keeping every real build action blocked: no build approval, no build validation approval, no package build, no ZIP, no installer, no release archive, no code signing, no checksum publication, no upload, and no distribution.
...
- Add this build-approval-freeze policy document.
- Add a local validation script.
- Create or refresh a local-only build-approval-freeze evidence directory outside the git repository.
- Write a local-only build-approval-freeze JSON and README into that evidence directory.
- Verify previous milestone markers and the v0.6.17 prebuild validation gate.
- Freeze build approval as a documented blocker only.
...
## Local build-approval-freeze evidence

The local build-approval-freeze evidence directory is intentionally outside the repository:

```text
D:\dev\release-assets\voila\v0.6.18-controlled-tester-package-build-approval-freeze-no-build-no-distribution
```
...
## Build approval freeze policy

Every build approval freeze entry must remain:
...
This milestone records build approval freeze only. It does not approve, enforce, copy, stage, build, package, sign, upload, or share any source.

## Decision

The only positive build-approval-freeze decision allowed here is:
