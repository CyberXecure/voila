...
Scope: owner-controlled release gate validation dry-run only
...
This milestone validates the v0.6.21 release gate plan as a dry-run only.

It confirms that planned release gate categories exist and remain unapproved before any future release approval freeze can be considered. It keeps every real action blocked: no release gate approval, no release gate enforcement, no tester delivery approval, no public release, no build readiness unfreeze, no package build, no ZIP, no installer, no release archive, no code signing, no checksum publication, no upload, and no distribution.
...
- Add this release-gate-validation dry-run policy document.
- Add a local validation script.
- Create or refresh a local-only release-gate-validation evidence directory outside the git repository.
- Write a local-only release-gate-validation JSON and README into that evidence directory.
- Verify previous milestone markers and the v0.6.21 release gate plan.
- Validate release gate categories as dry-run metadata only.
...
## Local release-gate-validation evidence

The local release-gate-validation evidence directory is intentionally outside the repository:

```text
D:\dev\release-assets\voila\v0.6.22-controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution
```
...
## Release gate validation policy

Every release gate validation entry must remain:
...
This milestone records release gate validation only. It does not approve, unfreeze, enforce, copy, stage, build, package, sign, upload, or share any source.

## Release gate categories validated as dry-run only
...
The only positive release-gate-validation decision allowed here is:
