...
Scope: owner-controlled staging layout validation dry-run only
...
This milestone validates the staging layout plan from v0.6.11 without creating a staging layout, creating a staging tree, copying runtime files, approving source copies, building, zipping, creating an installer, staging a release archive, or distributing anything.

It follows the v0.6.11 staging layout plan and records whether each planned layout entry and planned source mapping is still valid for a future staging-layout creation milestone.
...
- Add this staging-layout validation dry-run policy document.
- Add a local validation script.
- Create or refresh a local-only staging-layout validation evidence directory outside the git repository.
- Write a local-only staging-layout validation JSON and README into that evidence directory.
- Validate planned layout names and planned source mapping strings from v0.6.11.
- Confirm that every validated layout entry remains `layout_created_now = false`.
...
## Local staging-layout validation evidence

The local staging-layout validation evidence directory is intentionally outside the repository:

```text
D:\dev\release-assets\voila\v0.6.12-controlled-tester-package-staging-layout-validation-dry-run-no-create-no-copy-no-build-no-distribution
```
...
## Staging layout validation policy

The staging-layout validation dry-run may validate these planned layout names only:
...
It may validate these planned source mappings only:
...
- `scripts/dev/check-controlled-tester-package-build-plan-no-build-no-distribution.ps1 -> validation/check-controlled-tester-package-build-plan-no-build-no-distribution.ps1`

Every validated layout entry and mapping must remain:
...
This milestone records staging layout validation only. It does not approve, enforce, copy, stage, build, package, sign, upload, or share any source.

## Decision

The only positive staging-layout validation decision allowed here is:
