...
doc = root / "docs" / "dev" / "course-tools-normal-study-link-default-manual-study-audit-no-progress-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0829_WEB_APP_MISSING"),
    (doc, "FAILED_V0829_DOC_MISSING"),
]:
    if not path.exists():
...
for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0829_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Course Tools normal Study link → default Manual Study audit",
    ...
    "The normal Course Tools → Study path has owner-local browser readiness evidence",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0829_DOC_TERM_MISSING={term}")
...
allowed = {
    "docs/dev/course-tools-normal-study-link-default-manual-study-audit-no-progress-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-course-tools-normal-study-link-default-manual-study-audit-no-progress-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-course-tools-normal-study-link-default-manual-study-audit-no-progress-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0829_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0829_STUDY_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/study"') != 0:
    raise SystemExit("FAILED_V0829_STUDY_POST_ADDED")

if web_text.count("VOILA_V0_8_27_MANUAL_STUDY_DEFAULT_STUDY_READ_ONLY_FALLBACK_START") != 1:
    raise SystemExit("FAILED_V0829_DEFAULT_BLOCK_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0829_FORBIDDEN_WEB_TERM_FOUND={forbidden}")
...
if not study_route_match:
    raise SystemExit("FAILED_V0829_STUDY_ROUTE_STATIC_BLOCK_NOT_FOUND")
...
if study_route_hash != expected_v0821_study_route_hash:
    raise SystemExit(
        "FAILED_V0829_STUDY_ROUTE_HASH_CHANGED="
        + study_route_hash
        + ";EXPECTED="
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0829_POST={url}; STATUS={exc.code}; BODY={error_body}")
...
def fetch_url_allow_http_error(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0829_{label}_FETCH={last_error}")
...
eligible_id = "v0829-check-normal-study-link-default-manual-study-audit-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[282, 304, 740, 670]",
    "kind": "formula",
    "title": "v0.8.29 normal Study link default Manual Study eligible",
    "verified_text": "Normal Study link default Manual Study prompt text",
    "explanation_ro": "Răspuns read-only pentru auditul linkului normal Study din Course Tools.",
    "source_status": "verified",
    "source_note": "Created by v0.8.29 normal Study link default Manual Study audit check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0829_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0829_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0829_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0829_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0829_MANUAL_STUDY_PREVIEW_JSON_MISSING")

home_status, home_body, _ = fetch_url_allow_http_error(home_url, "HOME")
if home_status != 200:
    raise SystemExit(f"FAILED_V0829_HOME_STATUS={home_status}; BODY={home_body[:800]}")
...
if course_tools_status != 200:
    raise SystemExit(f"FAILED_V0829_COURSE_TOOLS_STATUS={course_tools_status}; BODY={course_tools_body[:800]}")
...
if not normal_study_hrefs:
    raise SystemExit("FAILED_V0829_NORMAL_STUDY_LINK_HREF_NOT_FOUND")
...
if normal_study_href is None:
    raise SystemExit("FAILED_V0829_NORMAL_STUDY_LINK_HREF_UNEXPECTED=" + ",".join(normal_study_hrefs[:10]))
...
if not shadow_link_match:
    raise SystemExit("FAILED_V0829_SHADOW_LINK_HREF_NOT_FOUND")
...
if not dry_run_link_match:
    raise SystemExit("FAILED_V0829_DRY_RUN_LINK_HREF_NOT_FOUND")
...
if normal_status != 200:
    raise SystemExit(f"FAILED_V0829_NORMAL_STUDY_STATUS={normal_status}; BODY={normal_body[:800]}")

if shadow_status != 200:
    raise SystemExit(f"FAILED_V0829_SHADOW_STATUS={shadow_status}; BODY={shadow_body[:800]}")

if dry_run_status != 200:
    raise SystemExit(f"FAILED_V0829_DRY_RUN_STATUS={dry_run_status}; BODY={dry_run_body[:800]}")
...
for term in course_tools_terms:
    if term not in course_tools_body:
        raise SystemExit(f"FAILED_V0829_COURSE_TOOLS_TERM_MISSING={term}")
...
normal_terms = [
    ...
    "Normal Study link default Manual Study prompt text",
    "Răspuns read-only pentru auditul linkului normal Study din Course Tools.",
]

for term in normal_terms:
    if term not in normal_body:
        raise SystemExit(f"FAILED_V0829_NORMAL_STUDY_TERM_MISSING={term}")
...
for term in shadow_terms:
    if term not in shadow_body:
        raise SystemExit(f"FAILED_V0829_SHADOW_TERM_MISSING={term}")
...
for term in dry_run_terms:
    if term not in dry_run_body:
        raise SystemExit(f"FAILED_V0829_DRY_RUN_TERM_MISSING={term}")

manual_study = json.loads(manual_study_preview.read_text(encoding="utf-8", errors="replace"))
items = manual_study.get("items")
if not isinstance(items, list) or not items:
    raise SystemExit("FAILED_V0829_ITEMS_INVALID_OR_EMPTY")

ids = {item.get("source_evidence_id") for item in items if isinstance(item, dict)}
if eligible_id not in ids:
    raise SystemExit("FAILED_V0829_ELIGIBLE_ID_NOT_EXPORTED")

fallback_status = None
fallback_body = ""
tmp_preview = manual_study_preview.with_name(manual_study_preview.name + ".v0829tmp")
...
if fallback_status != 200:
    raise SystemExit(f"FAILED_V0829_NORMAL_STUDY_LEGACY_FALLBACK_STATUS={fallback_status}; BODY={fallback_body[:800]}")

if "manual-study-default-route" in fallback_body:
    raise SystemExit("FAILED_V0829_NORMAL_STUDY_FALLBACK_RENDERED_MANUAL_DEFAULT_WHEN_PREVIEW_MISSING")
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0829_LEGACY_STUDY_PREVIEW_CHANGED")

manual_result = {
    "manual_browser_audit_status": "OWNER_LOCAL_COURSE_TOOLS_NORMAL_STUDY_LINK_DEFAULT_MANUAL_STUDY_PASS",
    "steps": [
        ...
        {"step": "shadow_link_preserved", "status": "PASS", "href": shadow_href},
        {"step": "dry_run_link_preserved", "status": "PASS", "href": dry_run_href},
        {"step": "shadow_route_still_loads", "status": "PASS", "http_status": shadow_status},
        {"step": "dry_run_route_still_loads", "status": "PASS", "http_status": dry_run_status},
...
summary = {
    "VOILA_V0_8_29_COURSE_TOOLS_NORMAL_STUDY_LINK_DEFAULT_MANUAL_STUDY_AUDIT_CHECK": "PASS",
    "depends_on_v0828_default_study_browser_readiness_audit": True,
    ...
    "course_tools_dry_run_link_still_visible": True,
    "course_tools_dry_run_link_href": dry_run_href,
    "shadow_route_still_loads": True,
    "shadow_route_status": shadow_status,
    "dry_run_route_still_loads": True,
    ...
    "manual_study_connected_to_real_study": "normal_course_tools_study_link_to_default_read_only_manual_study",
    ...
    "TESTER_READINESS": "BLOCKED_AUDIT_ONLY_NO_PROGRESS_NO_PACKAGE",
    "POLICY": "course_tools_normal_study_link_default_manual_study_audit_no_progress_no_build_no_zip_no_share_no_delivery_no_distribution",
}

evidence = Path(r"D:\dev\tester-runs\v0829-course-tools-normal-study-link-default-manual-study-audit")
evidence.mkdir(parents=True, exist_ok=True)

out_json = evidence / "V0.8.29-COURSE-TOOLS-NORMAL-STUDY-LINK-DEFAULT-MANUAL-STUDY-AUDIT-CHECK.json"
out_md = evidence / "V0.8.29-COURSE-TOOLS-NORMAL-STUDY-LINK-DEFAULT-MANUAL-STUDY-AUDIT-CHECK.md"
