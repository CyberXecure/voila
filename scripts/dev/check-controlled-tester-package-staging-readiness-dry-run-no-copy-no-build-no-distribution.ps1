...
Write-Host "=== v0.6.10 controlled tester package staging readiness dry-run no-copy no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-staging-readiness-dry-run-no-copy-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-staging-readiness-dry-run-no-copy-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$readinessRoot = "D:\dev\release-assets\voila\v0.6.10-controlled-tester-package-staging-readiness-dry-run-no-copy-no-build-no-distribution"
$readinessPath = Join-Path $readinessRoot "STAGING-READINESS-DRY-RUN.json"
$readmePath = Join-Path $readinessRoot "STAGING-READINESS-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.10 file: $path"
  Write-Host $path
}
...
$v069DocText = Get-Content -LiteralPath $previous.v069Doc -Raw
...
Assert-True ($v069DocText -match "v0\.6\.10-controlled-tester-package-staging-readiness-dry-run-no-copy-no-build-no-distribution") "v0.6.9 allowed next marker missing."
...
Assert-True ($v069Text -match "copy_validation_approved_now.*false") "v0.6.9 must keep copy validation approval blocked."
Assert-True ($v069Text -match "copy_allowed_now.*false") "v0.6.9 must keep copy allowed blocked."
Assert-True ($v069Text -match "copy_executed_now.*false") "v0.6.9 must keep copy execution blocked."
Assert-True ($v069Text -match "copied_now.*false") "v0.6.9 must keep copied_now false."
Assert-True ($v069Text -match "staging_tree_created.*false") "v0.6.9 must keep staging tree creation blocked."
Assert-True ($v069Text -match "copies_runtime_files.*false") "v0.6.9 must keep runtime copy blocked."
Assert-True ($v069Text -match "build_allowed.*false") "v0.6.9 must keep build blocked."
Assert-True ($v069Text -match "distribution_allowed.*false") "v0.6.9 must keep distribution blocked."
Assert-True ($v069Text -match "uploads_to_onedrive.*false") "v0.6.9 must keep OneDrive upload blocked."
Assert-True ($v069Text -match "publishes_github_release.*false") "v0.6.9 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.10 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.10 staging-readiness milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records staging readiness only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "staging_readiness_approved_now.*false") "Check must keep staging_readiness_approved_now false."
...
Write-Host ""
Write-Host "=== Create local-only staging readiness evidence directory ==="
New-Item -ItemType Directory -Force -Path $readinessRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$readinessFull = Resolve-FullPathSafe $readinessRoot
Assert-True (-not ($readinessFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Staging readiness root must be outside repository."
...
$readiness = [ordered]@{
  schema_version = "1"
  staging_readiness_version = "v0.6.10"
  staging_readiness_type = "controlled_tester_package_staging_readiness_dry_run_no_copy_no_build_no_distribution"
  status = "pass"
  staging_readiness = [ordered]@{
    owner_controlled_staging_readiness_dry_run_created = $true
    readiness_root = $readinessRoot
    readiness_root_outside_repo = $true
    readiness_json_created = $true
    readme_created = $true
    readiness_checked = $true
    staging_readiness_approved_now = $false
    staging_tree_created = $false
    staging_layout_created = $false
    copy_allowed_now = $false
    copy_executed_now = $false
    ...
    next_step_policy = "STOP_OR_SEPARATE_STAGING_LAYOUT_PLAN_NO_CREATE_MILESTONE_ONLY"
  }
  readiness_items = $readinessItems
  evidence = [ordered]@{
    previous_copy_validation_version = "v0.6.9"
    previous_copy_validation_check = $previous.v069Check
  ...
  gates = [ordered]@{
    ...
    staging_readiness_dry_run_only = $true
    readiness_root_outside_repo = $true
    readiness_checked = $true
    no_staging_readiness_approval_now = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_staging_readiness_evidence = $true
    approves_staging_readiness = $false
  ...
  allowed_next_milestones = @("STOP", "v0.6.11-controlled-tester-package-staging-layout-plan-no-create-no-copy-no-build-no-distribution")
...
[System.IO.File]::WriteAllText($readinessPath, ($readinessJson + "`n"), [System.Text.UTF8Encoding]::new($false))

$readme = @"
Voila v0.6.10 controlled tester package staging readiness dry-run no-copy no-build no-distribution
...
Blocked:
- staging tree creation, staging layout creation, runtime file copy, package content copy, copy execution, package source approval, source allowlist approval, source allowlist enforcement, package build, ZIP, EXE, MSI, installer, runnable package, release archive, release asset staging, code signing, checksum publication, public UI, tester activation, submit flow, persistence, scoring, cloud/API.
"@

[System.IO.File]::WriteAllText($readmePath, ($readme.TrimEnd() + "`n"), [System.Text.UTF8Encoding]::new($false))

Write-Host "STAGING READINESS ROOT: $readinessRoot"
Write-Host "WROTE: $readinessPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify staging readiness evidence files ==="
$readinessFiles = @(Get-ChildItem -LiteralPath $readinessRoot -File | Select-Object -ExpandProperty Name | Sort-Object)
foreach ($name in $readinessFiles) { Write-Host $name }

$allowedReadinessNames = @("STAGING-READINESS-DRY-RUN.json", "STAGING-READINESS-README.txt")
foreach ($name in $readinessFiles) {
  Assert-True ($allowedReadinessNames -contains $name) "Unexpected file in local-only staging readiness evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in staging readiness evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $readinessPath) "Missing staging-readiness JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing staging-readiness README."

Write-Host ""
Write-Host "=== v0.6.10 staging readiness JSON ==="
$readinessJson
...
Write-Host ""
Write-Host "=== v0.6.10 CONTROLLED TESTER PACKAGE STAGING READINESS DRY-RUN NO-COPY NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
