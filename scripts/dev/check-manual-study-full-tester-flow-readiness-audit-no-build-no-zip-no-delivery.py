...
doc = root / "docs" / "dev" / "manual-study-full-tester-flow-readiness-audit-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0832_WEB_APP_MISSING"),
    (doc, "FAILED_V0832_DOC_MISSING"),
]:
    if not path.exists():
...
for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0832_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Manual Study full tester-flow readiness audit",
    "before any package rebuild, ZIP creation, share, or delivery",
    ...
    "Home UI exposes the Course Tools entry for the existing course.",
    "Course Tools loads from the Home UI link with HTTP 200.",
    "Course Tools exposes the normal Study link to `/study?pdf={pdf_name}`.",
    "The normal Study link opens `/study?pdf={pdf_name}` with HTTP 200.",
    ...
    "The shadow route remains separate and loads with HTTP 200.",
    "Course Tools still exposes the separate dry-run link.",
    "The dry-run route remains separate and loads with HTTP 200.",
    "Normal Study falls back to legacy Study when `manual_study_items.preview.json` is temporarily missing.",
    ...
    "No crop file write happens.",
    "No build is performed.",
    "No ZIP is created.",
    "No share is created.",
    "No delivery is performed.",
    "No distribution is performed.",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0832_DOC_TERM_MISSING={term}")
...
allowed = {
    "docs/dev/manual-study-full-tester-flow-readiness-audit-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-manual-study-full-tester-flow-readiness-audit-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-manual-study-full-tester-flow-readiness-audit-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0832_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0832_STUDY_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/study"') != 0:
    raise SystemExit("FAILED_V0832_STUDY_POST_ADDED")

if web_text.count("VOILA_V0_8_27_MANUAL_STUDY_DEFAULT_STUDY_READ_ONLY_FALLBACK_START") != 1:
    raise SystemExit("FAILED_V0832_DEFAULT_BLOCK_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0832_FORBIDDEN_WEB_TERM_FOUND={forbidden}")
...
if not study_route_match:
    raise SystemExit("FAILED_V0832_STUDY_ROUTE_STATIC_BLOCK_NOT_FOUND")
...
if study_route_hash != expected_v0821_study_route_hash:
    raise SystemExit(
        "FAILED_V0832_STUDY_ROUTE_HASH_CHANGED="
        + study_route_hash
        + ";EXPECTED="
...
if not input_pdf.exists():
    raise SystemExit("FAILED_V0832_INPUT_PDF_MISSING=" + str(input_pdf))

if not output_dir.exists():
    raise SystemExit("FAILED_V0832_EXISTING_GENERATED_COURSE_OUTPUT_MISSING=" + str(output_dir))
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0832_POST={url}; STATUS={exc.code}; BODY={error_body}")
...
def fetch_url_allow_http_error(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0832_{label}_FETCH={last_error}")
...
eligible_id = "v0832-check-full-tester-flow-readiness-audit-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[312, 334, 800, 730]",
    "kind": "formula",
    "title": "v0.8.32 full tester flow readiness eligible",
    "verified_text": "Full tester flow readiness prompt text",
    "explanation_ro": "Răspuns read-only pentru auditul full tester-flow Manual Study.",
    "source_status": "verified",
    "source_note": "Created by v0.8.32 full tester-flow readiness audit check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0832_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0832_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0832_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0832_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0832_MANUAL_STUDY_PREVIEW_JSON_MISSING")

home_status, home_body, _ = fetch_url_allow_http_error(home_url, "HOME")
if home_status != 200:
    raise SystemExit(f"FAILED_V0832_HOME_STATUS={home_status}; BODY={home_body[:800]}")

if course_id not in home_body and pdf_name not in home_body:
    raise SystemExit("FAILED_V0832_HOME_DOES_NOT_REFERENCE_EXISTING_COURSE")
...
if not course_tools_hrefs:
    raise SystemExit("FAILED_V0832_HOME_COURSE_TOOLS_LINK_NOT_FOUND")
...
if course_tools_status != 200:
    raise SystemExit(f"FAILED_V0832_COURSE_TOOLS_FROM_HOME_STATUS={course_tools_status}; BODY={course_tools_body[:800]}")
...
if normal_study_href is None:
    raise SystemExit("FAILED_V0832_NORMAL_STUDY_LINK_HREF_NOT_FOUND_OR_UNEXPECTED=" + ",".join(normal_study_hrefs[:10]))
...
if not shadow_link_match:
    raise SystemExit("FAILED_V0832_SHADOW_LINK_HREF_NOT_FOUND")
...
if not dry_run_link_match:
    raise SystemExit("FAILED_V0832_DRY_RUN_LINK_HREF_NOT_FOUND")
...
if normal_status != 200:
    raise SystemExit(f"FAILED_V0832_NORMAL_STUDY_STATUS={normal_status}; BODY={normal_body[:800]}")

if shadow_status != 200:
    raise SystemExit(f"FAILED_V0832_SHADOW_STATUS={shadow_status}; BODY={shadow_body[:800]}")

if dry_run_status != 200:
    raise SystemExit(f"FAILED_V0832_DRY_RUN_STATUS={dry_run_status}; BODY={dry_run_body[:800]}")
...
normal_terms = [
    ...
    "Full tester flow readiness prompt text",
    "Răspuns read-only pentru auditul full tester-flow Manual Study.",
]

for term in normal_terms:
    if term not in normal_body:
        raise SystemExit(f"FAILED_V0832_NORMAL_STUDY_TERM_MISSING={term}")
...
for term in shadow_terms:
    if term not in shadow_body:
        raise SystemExit(f"FAILED_V0832_SHADOW_TERM_MISSING={term}")
...
for term in dry_run_terms:
    if term not in dry_run_body:
        raise SystemExit(f"FAILED_V0832_DRY_RUN_TERM_MISSING={term}")

manual_study = json.loads(manual_study_preview.read_text(encoding="utf-8", errors="replace"))
items = manual_study.get("items")
if not isinstance(items, list) or not items:
    raise SystemExit("FAILED_V0832_ITEMS_INVALID_OR_EMPTY")

ids = {item.get("source_evidence_id") for item in items if isinstance(item, dict)}
if eligible_id not in ids:
    raise SystemExit("FAILED_V0832_ELIGIBLE_ID_NOT_EXPORTED")

fallback_status = None
fallback_body = ""
tmp_preview = manual_study_preview.with_name(manual_study_preview.name + ".v0832tmp")
...
if fallback_status != 200:
    raise SystemExit(f"FAILED_V0832_NORMAL_STUDY_LEGACY_FALLBACK_STATUS={fallback_status}; BODY={fallback_body[:800]}")

if "manual-study-default-route" in fallback_body:
    raise SystemExit("FAILED_V0832_NORMAL_STUDY_FALLBACK_RENDERED_MANUAL_DEFAULT_WHEN_PREVIEW_MISSING")
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0832_LEGACY_STUDY_PREVIEW_CHANGED")

summary = {
    "VOILA_V0_8_32_MANUAL_STUDY_FULL_TESTER_FLOW_READINESS_AUDIT_CHECK": "PASS",
    "depends_on_v0831_tester_readiness_preflight_contract": True,
    ...
    "manual_tester_like_flow_ready_owner_local": True,
    "package_rebuild_approved": False,
    "extracted_package_validation_completed": False,
    "requires_separate_package_rebuild_milestone": True,
    ...
    "manual_study_connected_to_real_study": "full_tester_like_flow_default_manual_study_read_only_with_legacy_fallback",
    ...
    "TESTER_FLOW_READINESS": "PASS_OWNER_LOCAL_TESTER_LIKE_FLOW_ONLY",
    "PACKAGE_READINESS": "BLOCKED_NO_PACKAGE_REBUILD_NO_EXTRACTED_VALIDATION_NO_DELIVERY",
    "POLICY": "manual_study_full_tester_flow_readiness_audit_no_build_no_zip_no_share_no_delivery_no_distribution",
    "RECOMMENDED_NEXT": "v0.8.33-owner-local-tester-package-rebuild-preflight-no-build-no-zip-no-delivery",
}

evidence = Path(r"D:\dev\tester-runs\v0832-manual-study-full-tester-flow-readiness-audit")
evidence.mkdir(parents=True, exist_ok=True)

out_json = evidence / "V0.8.32-MANUAL-STUDY-FULL-TESTER-FLOW-READINESS-AUDIT-CHECK.json"
out_md = evidence / "V0.8.32-MANUAL-STUDY-FULL-TESTER-FLOW-READINESS-AUDIT-CHECK.md"

out_json.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

md_lines = [
    "# v0.8.32 Manual Study full tester-flow readiness audit",
    "",
    "## Summary",
]
