...
Write-Host "=== v0.6.16 controlled tester package prebuild decision gate no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-prebuild-decision-gate-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-prebuild-decision-gate-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$decisionRoot = "D:\dev\release-assets\voila\v0.6.16-controlled-tester-package-prebuild-decision-gate-no-build-no-distribution"
$decisionPath = Join-Path $decisionRoot "PREBUILD-DECISION-GATE-DRY-RUN.json"
$readmePath = Join-Path $decisionRoot "PREBUILD-DECISION-GATE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.16 file: $path"
  Write-Host $path
}
...
$v0615DocText = Get-Content -LiteralPath $previous.v0615Doc -Raw
...
Assert-True ($v0615DocText -match "v0\.6\.16-controlled-tester-package-prebuild-decision-gate-no-build-no-distribution") "v0.6.15 allowed next marker missing."

Assert-True ($v0615Text -match "gate_frozen.*true") "v0.6.15 must keep gate_frozen true."
Assert-True ($v0615Text -match "staging_gate_freeze_active.*true") "v0.6.15 must keep staging gate freeze active."
Assert-True ($v0615Text -match "staging_manifest_creation_allowed_now.*false") "v0.6.15 must keep staging manifest creation blocked."
Assert-True ($v0615Text -match "staging_manifest_approval_allowed_now.*false") "v0.6.15 must keep staging manifest approval blocked."
Assert-True ($v0615Text -match "staging_manifest_enforcement_allowed_now.*false") "v0.6.15 must keep staging manifest enforcement blocked."
Assert-True ($v0615Text -match "copy_allowed_now.*false") "v0.6.15 must keep copy allowed blocked."
Assert-True ($v0615Text -match "copy_executed_now.*false") "v0.6.15 must keep copy execution blocked."
Assert-True ($v0615Text -match "build_allowed.*false") "v0.6.15 must keep build blocked."
Assert-True ($v0615Text -match "distribution_allowed.*false") "v0.6.15 must keep distribution blocked."
Assert-True ($v0615Text -match "uploads_to_onedrive.*false") "v0.6.15 must keep OneDrive upload blocked."
Assert-True ($v0615Text -match "publishes_github_release.*false") "v0.6.15 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.16 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.16 prebuild-decision milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records a negative prebuild decision only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "prebuild_decision_gate_created.*true") "Check must set prebuild_decision_gate_created true."
Assert-True ($checkText -match "prebuild_decision_required.*true") "Check must set prebuild_decision_required true."
...
Write-Host ""
Write-Host "=== Create local-only prebuild decision evidence directory ==="
New-Item -ItemType Directory -Force -Path $decisionRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$decisionFull = Resolve-FullPathSafe $decisionRoot
Assert-True (-not ($decisionFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Prebuild decision root must be outside repository."
...
$decisionGateEntries = @()
foreach ($gateName in $decisionGates) {
  $decisionGateEntries += [ordered]@{
    gate_name = $gateName
    prebuild_decision_required = $true
...
$decision = [ordered]@{
  schema_version = "1"
  prebuild_decision_gate_version = "v0.6.16"
  prebuild_decision_gate_type = "controlled_tester_package_prebuild_decision_gate_no_build_no_distribution"
  status = "pass"
  prebuild_decision_gate = [ordered]@{
    owner_controlled_prebuild_decision_gate_created = $true
    prebuild_decision_gate_created = $true
    prebuild_decision_required = $true
    decision_root = $decisionRoot
    decision_root_outside_repo = $true
    decision_json_created = $true
    readme_created = $true
    build_approved_now = $false
    build_allowed_now = $false
    build_allowed = $false
    ...
    next_step_policy = "STOP_OR_SEPARATE_PREBUILD_VALIDATION_DRY_RUN_MILESTONE_ONLY"
  }
  decision_gates = $decisionGateEntries
  evidence = [ordered]@{
    previous_staging_gate_freeze_version = "v0.6.15"
    previous_staging_gate_freeze_check = $previous.v0615Check
  ...
  gates = [ordered]@{
    ...
    prebuild_decision_gate_only = $true
    decision_root_outside_repo = $true
    prebuild_decision_required = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_prebuild_decision_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.17-controlled-tester-package-prebuild-validation-dry-run-no-build-no-distribution")
...
[System.IO.File]::WriteAllText($decisionPath, ($decisionJson + "`n"), [System.Text.UTF8Encoding]::new($false))

$readme = @"
Voila v0.6.16 controlled tester package prebuild decision gate no-build no-distribution
...
Write-Host "PREBUILD DECISION ROOT: $decisionRoot"
Write-Host "WROTE: $decisionPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify prebuild decision evidence files ==="
$decisionFiles = @(Get-ChildItem -LiteralPath $decisionRoot -File | Select-Object -ExpandProperty Name | Sort-Object)
foreach ($name in $decisionFiles) { Write-Host $name }

$allowedDecisionNames = @("PREBUILD-DECISION-GATE-DRY-RUN.json", "PREBUILD-DECISION-GATE-README.txt")
foreach ($name in $decisionFiles) {
  Assert-True ($allowedDecisionNames -contains $name) "Unexpected file in local-only prebuild decision evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in prebuild decision evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $decisionPath) "Missing prebuild-decision JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing prebuild-decision README."

Write-Host ""
Write-Host "=== v0.6.16 prebuild decision JSON ==="
$decisionJson
...
Write-Host ""
Write-Host "=== v0.6.16 CONTROLLED TESTER PACKAGE PREBUILD DECISION GATE NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
