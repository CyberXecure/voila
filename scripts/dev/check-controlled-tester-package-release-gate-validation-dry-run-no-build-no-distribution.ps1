...
Write-Host "=== v0.6.22 controlled tester package release gate validation dry-run no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$validationRoot = "D:\dev\release-assets\voila\v0.6.22-controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution"
$validationPath = Join-Path $validationRoot "RELEASE-GATE-VALIDATION-DRY-RUN.json"
$readmePath = Join-Path $validationRoot "RELEASE-GATE-VALIDATION-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.22 file: $path"
  Write-Host $path
}
...
$v0621DocText = Get-Content -LiteralPath $previous.v0621Doc -Raw
...
Assert-True ($v0621DocText -match "v0\.6\.22-controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution") "v0.6.21 allowed next marker missing."

Assert-True ($v0621Text -match "release_gate_plan_created.*true") "v0.6.21 must have release gate plan."
Assert-True ($v0621Text -match "release_gate_plan_dry_run_only.*true") "v0.6.21 must keep release gate plan dry-run only."
Assert-True ($v0621Text -match "release_gate_approved_now.*false") "v0.6.21 must keep release gate approval blocked."
Assert-True ($v0621Text -match "release_gate_enforced_now.*false") "v0.6.21 must keep release gate enforcement blocked."
Assert-True ($v0621Text -match "public_release_allowed_now.*false") "v0.6.21 must keep public release blocked."
Assert-True ($v0621Text -match "tester_delivery_allowed_now.*false") "v0.6.21 must keep tester delivery blocked."
Assert-True ($v0621Text -match "distribution_allowed.*false") "v0.6.21 must keep distribution blocked."
Assert-True ($v0621Text -match "onedrive_upload_allowed_now.*false") "v0.6.21 must keep OneDrive upload blocked."
Assert-True ($v0621Text -match "github_release_allowed_now.*false") "v0.6.21 must keep GitHub release blocked."
Assert-True ($v0621Text -match "public_website_upload_allowed_now.*false") "v0.6.21 must keep public website upload blocked."
Assert-True ($v0621Text -match "package_build_executed_now.*false") "v0.6.21 must keep package build blocked."
Assert-True ($v0621Text -match "zip_created.*false") "v0.6.21 must keep zip creation blocked."
Assert-True ($v0621Text -match "release_archive_created.*false") "v0.6.21 must keep release archive blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.22 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.22 release-gate-validation milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "No live scoring",
  "release_gate_validation_created = true",
  ...
  "This milestone records release gate validation only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "release_gate_validation_created.*true") "Check must set release_gate_validation_created true."
Assert-True ($checkText -match "release_gate_validation_checked.*true") "Check must set release_gate_validation_checked true."
...
Write-Host ""
Write-Host "=== Create local-only release gate validation evidence directory ==="
New-Item -ItemType Directory -Force -Path $validationRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$validationFull = Resolve-FullPathSafe $validationRoot
Assert-True (-not ($validationFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Release gate validation root must be outside repository."

$releaseGateCategories = @(
...
$validationEntries = @()
foreach ($category in $releaseGateCategories) {
...
$validation = [ordered]@{
  schema_version = "1"
  release_gate_validation_version = "v0.6.22"
  release_gate_validation_type = "controlled_tester_package_release_gate_validation_dry_run_no_build_no_distribution"
  status = "pass"
  release_gate_validation = [ordered]@{
    owner_controlled_release_gate_validation_dry_run_created = $true
    release_gate_validation_created = $true
    release_gate_validation_checked = $true
    release_gate_validation_dry_run_only = $true
    ...
    next_step_policy = "STOP_OR_SEPARATE_RELEASE_APPROVAL_FREEZE_NO_BUILD_MILESTONE_ONLY"
  }
  validated_release_gate_categories = $validationEntries
  evidence = [ordered]@{
    previous_release_gate_plan_version = "v0.6.21"
    previous_release_gate_plan_check = $previous.v0621Check
  ...
  safety = [ordered]@{
    ...
    creates_local_release_gate_validation_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.23-controlled-tester-package-release-approval-freeze-no-build-no-distribution")
...
$readme = @"
Voila v0.6.22 controlled tester package release gate validation dry-run no-build no-distribution
...
Allowed contents:
- RELEASE-GATE-VALIDATION-README.txt
- RELEASE-GATE-VALIDATION-DRY-RUN.json

Blocked:
- release gate validation approval, release gate approval, release gate enforcement, public release approval, tester delivery approval, distribution approval, OneDrive delivery approval, GitHub release approval, public website upload approval, build readiness approval/unfreeze, build approval, build validation approval, package build, ZIP, EXE, MSI, installer, runnable package, staging manifest creation, staging layout creation, staging tree creation, runtime file copy, package content copy, copy execution, release archive creation, code signing, checksum publication, public UI, tester activation, submit flow, persistence, scoring, cloud/API.
"@

[System.IO.File]::WriteAllText($readmePath, ($readme.TrimEnd() + "`n"), [System.Text.UTF8Encoding]::new($false))

Write-Host "RELEASE GATE VALIDATION ROOT: $validationRoot"
Write-Host "WROTE: $validationPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify release gate validation evidence files ==="
...
$allowedValidationNames = @("RELEASE-GATE-VALIDATION-DRY-RUN.json", "RELEASE-GATE-VALIDATION-README.txt")
foreach ($name in $validationFiles) {
  Assert-True ($allowedValidationNames -contains $name) "Unexpected file in local-only release gate validation evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in release gate validation evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $validationPath) "Missing release-gate-validation JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing release-gate-validation README."

Write-Host ""
Write-Host "=== v0.6.22 release gate validation JSON ==="
$validationJson
...
Write-Host ""
Write-Host "=== v0.6.22 CONTROLLED TESTER PACKAGE RELEASE GATE VALIDATION DRY-RUN NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
