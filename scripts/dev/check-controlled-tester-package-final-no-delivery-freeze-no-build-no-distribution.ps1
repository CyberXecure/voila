...
Write-Host "=== v0.6.32 controlled tester package final no-delivery freeze no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-final-no-delivery-freeze-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-final-no-delivery-freeze-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$freezeRoot = "D:\dev\release-assets\voila\v0.6.32-controlled-tester-package-final-no-delivery-freeze-no-build-no-distribution"
$freezePath = Join-Path $freezeRoot "FINAL-NO-DELIVERY-FREEZE-DRY-RUN.json"
$readmePath = Join-Path $freezeRoot "FINAL-NO-DELIVERY-FREEZE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.32 file: $path"
  Write-Host $path
}
...
$previousFiles = @(
  "docs/dev/controlled-tester-package-delivery-approval-freeze-no-build-no-distribution.md",
  "scripts/dev/check-controlled-tester-package-delivery-approval-freeze-no-build-no-distribution.ps1",
...
$v0631Text = Get-Content -LiteralPath "scripts/dev/check-controlled-tester-package-delivery-approval-freeze-no-build-no-distribution.ps1" -Raw
$v0631DocText = Get-Content -LiteralPath "docs/dev/controlled-tester-package-delivery-approval-freeze-no-build-no-distribution.md" -Raw

Assert-True ($v0631Text -match "v0\.6\.31 CONTROLLED TESTER PACKAGE DELIVERY APPROVAL FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS") "v0.6.31 PASS marker missing."
Assert-True ($v0631DocText -match "v0\.6\.32-controlled-tester-package-final-no-delivery-freeze-no-build-no-distribution") "v0.6.31 allowed next marker missing."

Assert-True ($v0631Text -match "delivery_approval_freeze_active.*true") "v0.6.31 must keep delivery approval freeze active."
Assert-True ($v0631Text -match "delivery_approval_frozen.*true") "v0.6.31 must keep delivery approval frozen."
Assert-True ($v0631Text -match "delivery_readiness_validation_checked.*true") "v0.6.31 must keep delivery readiness validation checked."
Assert-True ($v0631Text -match "delivery_readiness_validation_dry_run_only.*true") "v0.6.31 must keep delivery readiness validation dry-run only."
Assert-True ($v0631Text -match "delivery_approval_allowed_now.*false") "v0.6.31 must keep delivery approval blocked."
Assert-True ($v0631Text -match "delivery_approval_unfreeze_allowed_now.*false") "v0.6.31 must keep delivery approval unfreeze blocked."
Assert-True ($v0631Text -match "delivery_execution_allowed_now.*false") "v0.6.31 must keep delivery execution blocked."
Assert-True ($v0631Text -match "controlled_tester_delivery_allowed_now.*false") "v0.6.31 must keep controlled tester delivery blocked."
Assert-True ($v0631Text -match "tester_activation_allowed_now.*false") "v0.6.31 must keep tester activation blocked."
Assert-True ($v0631Text -match "tester_notification_allowed_now.*false") "v0.6.31 must keep tester notification blocked."
Assert-True ($v0631Text -match "tester_access_grant_allowed_now.*false") "v0.6.31 must keep tester access grant blocked."
Assert-True ($v0631Text -match "onedrive_share_delivery_allowed_now.*false") "v0.6.31 must keep OneDrive share delivery blocked."
Assert-True ($v0631Text -match "onedrive_upload_allowed_now.*false") "v0.6.31 must keep OneDrive upload blocked."
Assert-True ($v0631Text -match "github_release_allowed_now.*false") "v0.6.31 must keep GitHub release blocked."
Assert-True ($v0631Text -match "public_website_upload_allowed_now.*false") "v0.6.31 must keep public website upload blocked."
Assert-True ($v0631Text -match "public_release_allowed_now.*false") "v0.6.31 must keep public release blocked."
Assert-True ($v0631Text -match "package_build_executed_now.*false") "v0.6.31 must keep package build blocked."
Assert-True ($v0631Text -match "zip_created.*false") "v0.6.31 must keep zip creation blocked."
Assert-True ($v0631Text -match "release_archive_created.*false") "v0.6.31 must keep release archive blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.32 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.32 final-no-delivery-freeze milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records final no-delivery freeze only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "final_no_delivery_freeze_created.*true") "Check must set final_no_delivery_freeze_created true."
Assert-True ($checkText -match "final_no_delivery_freeze_active.*true") "Check must set final_no_delivery_freeze_active true."
Assert-True ($checkText -match "final_no_delivery_frozen.*true") "Check must set final_no_delivery_frozen true."
Assert-True ($checkText -match "delivery_approval_freeze_active.*true") "Check must keep delivery_approval_freeze_active true."
Assert-True ($checkText -match "delivery_approval_frozen.*true") "Check must keep delivery_approval_frozen true."
...
Assert-True ($checkText -match '"STOP"') "Check must allow only STOP next."

Write-Host ""
Write-Host "=== Create local-only final no-delivery freeze evidence directory ==="
...
Assert-True (-not ($freezeFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Final no-delivery freeze root must be outside repository."

$frozenCategories = @(
  "delivery_approval_freeze",
  "delivery_readiness_validation_dry_run",
  "controlled_tester_delivery_execution",
  "tester_activation",
  "tester_notification",
  "tester_access_grant",
  "onedrive_share_delivery",
  "onedrive_upload_and_permission_change",
  "github_release_publication",
  "public_website_upload",
  "package_distribution",
  "release_approval_unfreeze",
  "build_approval_and_package_build",
  "zip_exe_msi_installer_creation",
  "code_signing_and_checksum_publication",
  "public_ui_and_tester_ui",
  "persistence_scoring_cloud_api"
)
...
$freeze = [ordered]@{
  schema_version = "1"
  final_no_delivery_freeze_version = "v0.6.32"
  final_no_delivery_freeze_type = "controlled_tester_package_final_no_delivery_freeze_no_build_no_distribution"
  status = "pass"
  final_no_delivery_freeze = [ordered]@{
    ...
    next_step_policy = "STOP_ONLY"
  }
  frozen_final_categories = $frozenEntries
  evidence = [ordered]@{
    previous_delivery_approval_freeze_version = "v0.6.31"
    previous_delivery_approval_freeze_check = "scripts/dev/check-controlled-tester-package-delivery-approval-freeze-no-build-no-distribution.ps1"
  ...
  gates = [ordered]@{
    ...
    final_no_delivery_freeze_only = $true
    freeze_root_outside_repo = $true
    final_no_delivery_freeze_created = $true
    final_no_delivery_freeze_active = $true
    final_no_delivery_frozen = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_final_no_delivery_freeze_evidence = $true
    freezes_final_no_delivery = $true
  ...
  allowed_next_milestones = @("STOP")
  blocked_next_actions = @(
    "any_v0_6_x_continuation",
...
$readme = @"
Voila v0.6.32 controlled tester package final no-delivery freeze no-build no-distribution
...
$allowedFreezeNames = @("FINAL-NO-DELIVERY-FREEZE-DRY-RUN.json", "FINAL-NO-DELIVERY-FREEZE-README.txt")
foreach ($name in $freezeFiles) {
  Assert-True ($allowedFreezeNames -contains $name) "Unexpected file in local-only final no-delivery freeze evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in final no-delivery freeze evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $freezePath) "Missing final-no-delivery-freeze JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing final-no-delivery-freeze README."

Write-Host ""
Write-Host "=== v0.6.32 final no-delivery freeze JSON ==="
$freezeJson
...
Write-Host ""
Write-Host "=== v0.6.32 CONTROLLED TESTER PACKAGE FINAL NO-DELIVERY FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
