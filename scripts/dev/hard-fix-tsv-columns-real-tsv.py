...
# 1. Replace TSV runner completely.
...
replacement = r'''def run_tesseract_tsv(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Important:
        # Do NOT pass "tsv" as final config file, because TESSDATA_PREFIX points to .tessdata
        # and that folder only contains language files. Use tessedit_create_tsv instead.
        cmd = [
            tesseract,
            str(image_path),
            str(output_base),
...
# 2. Replace TSV parser completely.
...
# 4. Ensure parse_words uses min_conf.
text = re.sub(
...
# 5. Add TSV preview before parsing, if missing.
...
# 6. Add report diagnostics, if missing.
...
print("OK: hard patched real TSV generation, parser, min-conf, and diagnostics.")
