...
Write-Host "=== v0.6.7 controlled tester package source allowlist plan no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-source-allowlist-plan-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-source-allowlist-plan-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$allowlistRoot = "D:\dev\release-assets\voila\v0.6.7-controlled-tester-package-source-allowlist-plan-no-build-no-distribution"
$allowlistPath = Join-Path $allowlistRoot "SOURCE-ALLOWLIST-PLAN-DRY-RUN.json"
$readmePath = Join-Path $allowlistRoot "SOURCE-ALLOWLIST-PLAN-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.7 file: $path"
  Write-Host $path
}
...
$v066DocText = Get-Content -LiteralPath $previous.v066Doc -Raw
...
Assert-True ($v066DocText -match "v0\.6\.7-controlled-tester-package-source-allowlist-plan-no-build-no-distribution") "v0.6.6 allowed next marker missing."
...
Assert-True ($v066Text -match "source_validation_approved_now.*false") "v0.6.6 must keep source validation approval blocked."
Assert-True ($v066Text -match "source_allowlist_approved_now.*false") "v0.6.6 must keep source allowlist approval blocked."
Assert-True ($v066Text -match "approved_for_package_now.*false") "v0.6.6 must keep package approval blocked."
Assert-True ($v066Text -match "copies_runtime_files.*false") "v0.6.6 must keep runtime copy blocked."
Assert-True ($v066Text -match "build_allowed.*false") "v0.6.6 must keep build blocked."
Assert-True ($v066Text -match "distribution_allowed.*false") "v0.6.6 must keep distribution blocked."
Assert-True ($v066Text -match "uploads_to_onedrive.*false") "v0.6.6 must keep OneDrive upload blocked."
Assert-True ($v066Text -match "publishes_github_release.*false") "v0.6.6 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.7 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.7 source-allowlist-plan milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records allowlist intent only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "source_allowlist_plan_approved_now.*false") "Check must keep source_allowlist_plan_approved_now false."
Assert-True ($checkText -match "source_allowlist_enforced_now.*false") "Check must keep source_allowlist_enforced_now false."
Assert-True ($checkText -match "approved_for_package_now.*false") "Check must keep approved_for_package_now false."
Assert-True ($checkText -match "approved_now.*false") "Check must keep approved_now false."
Assert-True ($checkText -match "enforced_now.*false") "Check must keep enforced_now false."
...
Write-Host ""
Write-Host "=== Create local-only source allowlist plan evidence directory ==="
New-Item -ItemType Directory -Force -Path $allowlistRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$allowlistFull = Resolve-FullPathSafe $allowlistRoot
Assert-True (-not ($allowlistFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Source allowlist plan root must be outside repository."
...
foreach ($candidatePath in $candidatePaths) {
  $plannedEntries += [ordered]@{
    ...
    exists = [bool](Test-Path -LiteralPath $candidatePath)
    path_kind = Get-PathKind $candidatePath
    planned_only = $true
    approved_now = $false
    enforced_now = $false
    copied_now = $false
...
$allowlist = [ordered]@{
  schema_version = "1"
  allowlist_plan_version = "v0.6.7"
  allowlist_plan_type = "controlled_tester_package_source_allowlist_plan_no_build_no_distribution"
  status = "pass"
  source_allowlist_plan = [ordered]@{
    owner_controlled_source_allowlist_plan_created = $true
    allowlist_root = $allowlistRoot
    allowlist_root_outside_repo = $true
    allowlist_json_created = $true
    readme_created = $true
    source_allowlist_plan_approved_now = $false
    source_allowlist_enforced_now = $false
    source_validation_approved_now = $false
    source_allowlist_approved_now = $false
    ...
    next_step_policy = "STOP_OR_SEPARATE_COPY_PLAN_NO_COPY_MILESTONE_ONLY"
  }
  planned_allowlist_entries = $plannedEntries
  evidence = [ordered]@{
    previous_source_validation_version = "v0.6.6"
    previous_source_validation_check = $previous.v066Check
  ...
  gates = [ordered]@{
    ...
    source_allowlist_plan_only = $true
    allowlist_root_outside_repo = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_source_allowlist_plan_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.8-controlled-tester-package-copy-plan-no-copy-no-build-no-distribution")
...
$allowlistJson = $allowlist | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText($allowlistPath, ($allowlistJson + "`n"), [System.Text.UTF8Encoding]::new($false))

$readme = @"
Voila v0.6.7 controlled tester package source allowlist plan no-build no-distribution
...
Blocked:
- runtime file copy, package source approval, source allowlist approval, source allowlist enforcement, package build, ZIP, EXE, MSI, installer, runnable package, release archive, release asset staging, code signing, checksum publication, public UI, tester activation, submit flow, persistence, scoring, cloud/API.
"@

[System.IO.File]::WriteAllText($readmePath, ($readme.TrimEnd() + "`n"), [System.Text.UTF8Encoding]::new($false))

Write-Host "SOURCE ALLOWLIST PLAN ROOT: $allowlistRoot"
Write-Host "WROTE: $allowlistPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify source allowlist plan evidence files ==="
$allowlistFiles = @(Get-ChildItem -LiteralPath $allowlistRoot -File | Select-Object -ExpandProperty Name | Sort-Object)
foreach ($name in $allowlistFiles) { Write-Host $name }

$allowedAllowlistNames = @("SOURCE-ALLOWLIST-PLAN-DRY-RUN.json", "SOURCE-ALLOWLIST-PLAN-README.txt")
foreach ($name in $allowlistFiles) {
  Assert-True ($allowedAllowlistNames -contains $name) "Unexpected file in local-only source allowlist plan evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in source allowlist plan evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $allowlistPath) "Missing source-allowlist plan JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing source-allowlist plan README."

Write-Host ""
Write-Host "=== v0.6.7 source allowlist plan JSON ==="
$allowlistJson
...
Write-Host ""
Write-Host "=== v0.6.7 CONTROLLED TESTER PACKAGE SOURCE ALLOWLIST PLAN NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
