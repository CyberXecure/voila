...
Write-Host "=== LOCAL BANK FIRST LIVE TRIAL OWNER SMOKE ROUTE CHECK v0.4.85 ==="
...
$TempPy = Join-Path $env:TEMP "voila-v085-owner-smoke-route-check.py"
...
try {
  $pyLines = @(
    ...
    'response = client.get("/exam-prep/local-bank/first-live-trial-owner-smoke")',
  ...
  if ($LASTEXITCODE -ne 0) { throw "owner smoke disabled sample failed" }
  $DisabledStatusCode = [int]$DisabledRaw[0]
  $DisabledJson = $DisabledRaw[1]
  Write-Host "disabled_owner_smoke_status_code $DisabledStatusCode"
  ...
  if ($LASTEXITCODE -ne 0) { throw "owner smoke enabled sample failed" }
  $EnabledStatusCode = [int]$EnabledRaw[0]
  $EnabledJson = $EnabledRaw[1]
  Write-Host "enabled_owner_smoke_status_code $EnabledStatusCode"
  ...
  $version_ok = ($Disabled.owner_smoke_route_version -eq "v0.4.85" -and $Enabled.owner_smoke_route_version -eq "v0.4.85")
  $mode_ok = ($Enabled.mode -eq "guarded_first_live_trial_owner_smoke_route")
  ...
  $smoke_ok = (
    $Enabled.smoke_status -eq "owner_smoke_ready_for_next_decision" -and
    $Enabled.owner_smoke_ready_for_next_decision -eq $true -and
    $Enabled.adapter_ready_for_owner_review -eq $true -and
    $Enabled.no_persistence_delivery_contract_ready -eq $true
  )
  $checks_ok = (
    $Enabled.smoke_checks.adapter_ready -eq $true -and
    $Enabled.smoke_checks.delivery_attempted_false -eq $true -and
    $Enabled.smoke_checks.delivery_performed_false -eq $true -and
    $Enabled.smoke_checks.delivered_question_count_zero -eq $true -and
    $Enabled.smoke_checks.blocks_delivery_now -eq $true -and
    $Enabled.smoke_checks.returns_noop_result -eq $true
  )
  $source_ok = (
  ...
  $next_ok = (@($Enabled.next_decision_options).Count -ge 3)
  $implementation_ok = (
    $Enabled.implementation_scope.json_only_owner_smoke_route -eq $true -and
