...
Write-Host "=== v0.5.8 owner preview visual/copy polish check ===" -ForegroundColor Cyan

$ApiPath = Join-Path $RepoRoot "services\api"
$env:PYTHONPATH = $ApiPath

Write-Host "`n=== Static page polish checks ===" -ForegroundColor Yellow
...
$requiredMarkers = @(
  "_voila_owner_session_preview_page_polish_display_copy",
  "_voila_owner_session_preview_page_question_type_label",
  "_voila_owner_session_preview_page_difficulty_label",
  "Previzualizare sesiune Pregătire examene",
  "Răspunsul și explicația sunt ascunse"
)

foreach ($marker in $requiredMarkers) {
  if ($pageBlock -notmatch [regex]::Escape($marker)) {
    throw "Missing v0.5.8 page polish marker: $marker"
  }
}
...
if ($content -match "href=.*/owner/exam-prep/session-preview") {
  throw "Hidden owner page appears linked from public UI"
}

Write-Host "`n=== Dynamic page polish checks ===" -ForegroundColor Yellow
...
required = [
    ...
    "răspuns scurt",
    "mediu",
    "Menționează o idee-cheie susținută",
    "Dacă există lim",
    "L&#x27;Hôpital data de",
    "Răspunsul și explicația sunt ascunse",
    "nu salvează încercări",
    "nu actualizează progresul",
    "nu calculează scoruri",
]

for needle in required:
    if needle not in html:
        raise SystemExit(f"Missing expected polished page text: {needle}")
...
for forbidden in [
    ...
    "Daca exista",
    "LHopital",
    "Answer and explanation are hidden",
    "<form",
    "<input",
    "<button",
    "correct_answer",
...
question_cards = html.count('class="question-card"')
if question_cards != 5:
    raise SystemExit(f"Expected exactly 5 rendered question cards, found {question_cards}")
...
if ($changedText -match "templates|static|assets") {
  throw "Unexpected public UI/template/static asset change in v0.5.8"
}
...
Write-Host "`n=== v0.5.8 OWNER PREVIEW VISUAL/COPY POLISH CHECK PASS ===" -ForegroundColor Green
