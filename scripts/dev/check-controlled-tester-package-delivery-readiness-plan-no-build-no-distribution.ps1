...
Write-Host "=== v0.6.29 controlled tester package delivery readiness plan no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-delivery-readiness-plan-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-delivery-readiness-plan-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$planRoot = "D:\dev\release-assets\voila\v0.6.29-controlled-tester-package-delivery-readiness-plan-no-build-no-distribution"
$planPath = Join-Path $planRoot "DELIVERY-READINESS-PLAN-DRY-RUN.json"
$readmePath = Join-Path $planRoot "DELIVERY-READINESS-PLAN-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.29 file: $path"
  Write-Host $path
}
...
$v0628Text = Get-Content -LiteralPath "scripts/dev/check-controlled-tester-package-distribution-approval-freeze-no-build-no-distribution.ps1" -Raw
$v0628DocText = Get-Content -LiteralPath "docs/dev/controlled-tester-package-distribution-approval-freeze-no-build-no-distribution.md" -Raw

Assert-True ($v0628Text -match "v0\.6\.28 CONTROLLED TESTER PACKAGE DISTRIBUTION APPROVAL FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS") "v0.6.28 PASS marker missing."
Assert-True ($v0628DocText -match "v0\.6\.29-controlled-tester-package-delivery-readiness-plan-no-build-no-distribution") "v0.6.28 allowed next marker missing."

Assert-True ($v0628Text -match "distribution_approval_freeze_active.*true") "v0.6.28 must keep distribution approval freeze active."
Assert-True ($v0628Text -match "distribution_approval_frozen.*true") "v0.6.28 must keep distribution approval frozen."
Assert-True ($v0628Text -match "distribution_readiness_validation_checked.*true") "v0.6.28 must keep distribution readiness validation checked."
Assert-True ($v0628Text -match "distribution_readiness_validation_dry_run_only.*true") "v0.6.28 must keep distribution readiness validation dry-run only."
Assert-True ($v0628Text -match "distribution_approval_allowed_now.*false") "v0.6.28 must keep distribution approval blocked."
Assert-True ($v0628Text -match "distribution_approval_unfreeze_allowed_now.*false") "v0.6.28 must keep distribution approval unfreeze blocked."
Assert-True ($v0628Text -match "distribution_allowed.*false") "v0.6.28 must keep distribution blocked."
Assert-True ($v0628Text -match "package_distribution_allowed_now.*false") "v0.6.28 must keep package distribution blocked."
Assert-True ($v0628Text -match "tester_delivery_allowed_now.*false") "v0.6.28 must keep tester delivery blocked."
Assert-True ($v0628Text -match "controlled_tester_delivery_allowed_now.*false") "v0.6.28 must keep controlled tester delivery blocked."
Assert-True ($v0628Text -match "onedrive_share_delivery_allowed_now.*false") "v0.6.28 must keep OneDrive share delivery blocked."
Assert-True ($v0628Text -match "onedrive_upload_allowed_now.*false") "v0.6.28 must keep OneDrive upload blocked."
Assert-True ($v0628Text -match "github_release_allowed_now.*false") "v0.6.28 must keep GitHub release blocked."
Assert-True ($v0628Text -match "public_website_upload_allowed_now.*false") "v0.6.28 must keep public website upload blocked."
Assert-True ($v0628Text -match "public_release_allowed_now.*false") "v0.6.28 must keep public release blocked."
Assert-True ($v0628Text -match "package_build_executed_now.*false") "v0.6.28 must keep package build blocked."
Assert-True ($v0628Text -match "zip_created.*false") "v0.6.28 must keep zip creation blocked."
Assert-True ($v0628Text -match "release_archive_created.*false") "v0.6.28 must keep release archive blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.29 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.29 delivery-readiness-plan milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records a delivery readiness plan only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "delivery_readiness_plan_created.*true") "Check must set delivery_readiness_plan_created true."
Assert-True ($checkText -match "delivery_readiness_plan_dry_run_only.*true") "Check must set delivery_readiness_plan_dry_run_only true."
...
Write-Host ""
Write-Host "=== Create local-only delivery readiness plan evidence directory ==="
...
Assert-True (-not ($planFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Delivery readiness plan root must be outside repository."
...
$plan = [ordered]@{
  schema_version = "1"
  delivery_readiness_plan_version = "v0.6.29"
  delivery_readiness_plan_type = "controlled_tester_package_delivery_readiness_plan_no_build_no_distribution"
  status = "pass"
  delivery_readiness_plan = [ordered]@{
    ...
    next_step_policy = "STOP_OR_SEPARATE_DELIVERY_READINESS_VALIDATION_DRY_RUN_NO_BUILD_MILESTONE_ONLY"
  }
  planned_delivery_readiness_categories = $plannedEntries
  evidence = [ordered]@{
    previous_distribution_approval_freeze_version = "v0.6.28"
    previous_distribution_approval_freeze_check = "scripts/dev/check-controlled-tester-package-distribution-approval-freeze-no-build-no-distribution.ps1"
  ...
  gates = [ordered]@{
    ...
    delivery_readiness_plan_only = $true
    plan_root_outside_repo = $true
    delivery_readiness_plan_created = $true
    delivery_readiness_plan_dry_run_only = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_delivery_readiness_plan_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.30-controlled-tester-package-delivery-readiness-validation-dry-run-no-build-no-distribution")
...
$readme = @"
Voila v0.6.29 controlled tester package delivery readiness plan no-build no-distribution
...
$allowedPlanNames = @("DELIVERY-READINESS-PLAN-DRY-RUN.json", "DELIVERY-READINESS-PLAN-README.txt")
foreach ($name in $planFiles) {
  Assert-True ($allowedPlanNames -contains $name) "Unexpected file in local-only delivery readiness plan evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in delivery readiness plan evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $planPath) "Missing delivery-readiness-plan JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing delivery-readiness-plan README."

Write-Host ""
Write-Host "=== v0.6.29 delivery readiness plan JSON ==="
$planJson
...
Write-Host ""
Write-Host "=== v0.6.29 CONTROLLED TESTER PACKAGE DELIVERY READINESS PLAN NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
