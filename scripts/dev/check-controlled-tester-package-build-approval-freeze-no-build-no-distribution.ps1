...
Write-Host "=== v0.6.18 controlled tester package build approval freeze no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-build-approval-freeze-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-build-approval-freeze-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$freezeRoot = "D:\dev\release-assets\voila\v0.6.18-controlled-tester-package-build-approval-freeze-no-build-no-distribution"
$freezePath = Join-Path $freezeRoot "BUILD-APPROVAL-FREEZE-DRY-RUN.json"
$readmePath = Join-Path $freezeRoot "BUILD-APPROVAL-FREEZE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.18 file: $path"
  Write-Host $path
}
...
$v0617DocText = Get-Content -LiteralPath $previous.v0617Doc -Raw
...
Assert-True ($v0617DocText -match "v0\.6\.18-controlled-tester-package-build-approval-freeze-no-build-no-distribution") "v0.6.17 allowed next marker missing."

Assert-True ($v0617Text -match "validation_checked.*true") "v0.6.17 must validate prebuild."
Assert-True ($v0617Text -match "prebuild_decision_required.*true") "v0.6.17 must require prebuild decision."
Assert-True ($v0617Text -match "build_approval_validated_blocked.*true") "v0.6.17 must validate blocked build approval."
Assert-True ($v0617Text -match "build_validation_approved_now.*false") "v0.6.17 must keep build validation approval blocked."
Assert-True ($v0617Text -match "build_approved_now.*false") "v0.6.17 must keep build approval blocked."
Assert-True ($v0617Text -match "build_allowed_now.*false") "v0.6.17 must keep build allowed blocked."
Assert-True ($v0617Text -match "package_build_executed_now.*false") "v0.6.17 must keep package build execution blocked."
Assert-True ($v0617Text -match "zip_created.*false") "v0.6.17 must keep zip creation blocked."
Assert-True ($v0617Text -match "installer_created.*false") "v0.6.17 must keep installer creation blocked."
Assert-True ($v0617Text -match "release_archive_created.*false") "v0.6.17 must keep release archive creation blocked."
Assert-True ($v0617Text -match "distribution_allowed.*false") "v0.6.17 must keep distribution blocked."
Assert-True ($v0617Text -match "tester_delivery_allowed_now.*false") "v0.6.17 must keep tester delivery blocked."
Assert-True ($v0617Text -match "public_release_allowed_now.*false") "v0.6.17 must keep public release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.18 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.18 build-approval-freeze milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "No live scoring",
  "build_approval_freeze_created = true",
  "build_approval_freeze_active = true",
  "build_approval_frozen = true",
  ...
  "This milestone records build approval freeze only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "build_approval_freeze_created.*true") "Check must set build_approval_freeze_created true."
Assert-True ($checkText -match "build_approval_freeze_active.*true") "Check must set build_approval_freeze_active true."
Assert-True ($checkText -match "build_approval_frozen.*true") "Check must set build_approval_frozen true."
...
Write-Host ""
Write-Host "=== Create local-only build approval freeze evidence directory ==="
...
Assert-True (-not ($freezeFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Build approval freeze root must be outside repository."
...
$freezeEntries = @()
foreach ($gateName in $freezeGates) {
  $freezeEntries += [ordered]@{
    gate_name = $gateName
    build_approval_freeze_active = $true
    build_approval_frozen = $true
...
$freeze = [ordered]@{
  schema_version = "1"
  build_approval_freeze_version = "v0.6.18"
  build_approval_freeze_type = "controlled_tester_package_build_approval_freeze_no_build_no_distribution"
  status = "pass"
  build_approval_freeze = [ordered]@{
    owner_controlled_build_approval_freeze_created = $true
    build_approval_freeze_created = $true
    build_approval_freeze_active = $true
    build_approval_frozen = $true
    ...
    next_step_policy = "STOP_OR_SEPARATE_BUILD_READINESS_REVIEW_DRY_RUN_MILESTONE_ONLY"
  }
  frozen_build_approval_gates = $freezeEntries
  evidence = [ordered]@{
    previous_prebuild_validation_version = "v0.6.17"
    previous_prebuild_validation_check = $previous.v0617Check
  ...
  gates = [ordered]@{
    ...
    build_approval_freeze_only = $true
    freeze_root_outside_repo = $true
    build_approval_freeze_active = $true
    build_approval_frozen = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_build_approval_freeze_evidence = $true
    freezes_build_approval = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.19-controlled-tester-package-build-readiness-review-dry-run-no-build-no-distribution")
...
$readme = @"
Voila v0.6.18 controlled tester package build approval freeze no-build no-distribution
...
Allowed contents:
- BUILD-APPROVAL-FREEZE-README.txt
- BUILD-APPROVAL-FREEZE-DRY-RUN.json
...
Write-Host "BUILD APPROVAL FREEZE ROOT: $freezeRoot"
Write-Host "WROTE: $freezePath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify build approval freeze evidence files ==="
...
$allowedFreezeNames = @("BUILD-APPROVAL-FREEZE-DRY-RUN.json", "BUILD-APPROVAL-FREEZE-README.txt")
foreach ($name in $freezeFiles) {
  Assert-True ($allowedFreezeNames -contains $name) "Unexpected file in local-only build approval freeze evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in build approval freeze evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $freezePath) "Missing build-approval-freeze JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing build-approval-freeze README."

Write-Host ""
Write-Host "=== v0.6.18 build approval freeze JSON ==="
$freezeJson
...
Write-Host ""
Write-Host "=== v0.6.18 CONTROLLED TESTER PACKAGE BUILD APPROVAL FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
