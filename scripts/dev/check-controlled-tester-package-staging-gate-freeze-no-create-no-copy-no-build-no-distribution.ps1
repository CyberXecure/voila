...
Write-Host "=== v0.6.15 controlled tester package staging gate freeze no-create no-copy no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-staging-gate-freeze-no-create-no-copy-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-staging-gate-freeze-no-create-no-copy-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$freezeRoot = "D:\dev\release-assets\voila\v0.6.15-controlled-tester-package-staging-gate-freeze-no-create-no-copy-no-build-no-distribution"
$freezePath = Join-Path $freezeRoot "STAGING-GATE-FREEZE-DRY-RUN.json"
$readmePath = Join-Path $freezeRoot "STAGING-GATE-FREEZE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.15 file: $path"
  Write-Host $path
}
...
$v0614DocText = Get-Content -LiteralPath $previous.v0614Doc -Raw
...
Assert-True ($v0614DocText -match "v0\.6\.15-controlled-tester-package-staging-gate-freeze-no-create-no-copy-no-build-no-distribution") "v0.6.14 allowed next marker missing."

Assert-True ($v0614Text -match "staging_manifest_validation_approved_now.*false") "v0.6.14 must keep staging manifest validation approval blocked."
Assert-True ($v0614Text -match "manifest_validation_approved_now.*false") "v0.6.14 must keep manifest validation approval blocked."
Assert-True ($v0614Text -match "staging_manifest_enforced_now.*false") "v0.6.14 must keep staging manifest enforcement blocked."
Assert-True ($v0614Text -match "manifest_approved_now.*false") "v0.6.14 must keep manifest approval blocked."
Assert-True ($v0614Text -match "manifest_created_now.*false") "v0.6.14 must keep manifest creation blocked."
Assert-True ($v0614Text -match "staging_manifest_created.*false") "v0.6.14 must keep staging manifest creation blocked."
Assert-True ($v0614Text -match "staging_layout_created.*false") "v0.6.14 must keep staging layout creation blocked."
Assert-True ($v0614Text -match "staging_tree_created.*false") "v0.6.14 must keep staging tree creation blocked."
Assert-True ($v0614Text -match "staging_root_created.*false") "v0.6.14 must keep staging root creation blocked."
Assert-True ($v0614Text -match "copy_allowed_now.*false") "v0.6.14 must keep copy allowed blocked."
Assert-True ($v0614Text -match "copy_executed_now.*false") "v0.6.14 must keep copy execution blocked."
Assert-True ($v0614Text -match "copies_runtime_files.*false") "v0.6.14 must keep runtime copy blocked."
Assert-True ($v0614Text -match "build_allowed.*false") "v0.6.14 must keep build blocked."
Assert-True ($v0614Text -match "distribution_allowed.*false") "v0.6.14 must keep distribution blocked."
Assert-True ($v0614Text -match "uploads_to_onedrive.*false") "v0.6.14 must keep OneDrive upload blocked."
Assert-True ($v0614Text -match "publishes_github_release.*false") "v0.6.14 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.15 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.15 staging-gate-freeze milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records a freeze only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "staging_gate_freeze_active.*true") "Check must set staging_gate_freeze_active true."
Assert-True ($checkText -match "gate_frozen.*true") "Check must set gate_frozen true."
Assert-True ($checkText -match "staging_manifest_creation_allowed_now.*false") "Check must keep staging_manifest_creation_allowed_now false."
Assert-True ($checkText -match "staging_manifest_approval_allowed_now.*false") "Check must keep staging_manifest_approval_allowed_now false."
Assert-True ($checkText -match "staging_manifest_enforcement_allowed_now.*false") "Check must keep staging_manifest_enforcement_allowed_now false."
...
Write-Host ""
Write-Host "=== Create local-only staging gate freeze evidence directory ==="
...
Assert-True (-not ($freezeFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Staging gate freeze root must be outside repository."
...
$frozenGateEntries = @()
foreach ($gateName in $frozenGates) {
  $frozenGateEntries += [ordered]@{
    gate_name = $gateName
    gate_frozen = $true
    staging_gate_freeze_active = $true
    allowed_now = $false
...
$freeze = [ordered]@{
  schema_version = "1"
  staging_gate_freeze_version = "v0.6.15"
  staging_gate_freeze_type = "controlled_tester_package_staging_gate_freeze_no_create_no_copy_no_build_no_distribution"
  status = "pass"
  staging_gate_freeze = [ordered]@{
    ...
    next_step_policy = "STOP_OR_SEPARATE_PREBUILD_DECISION_GATE_MILESTONE_ONLY"
  }
  frozen_gates = $frozenGateEntries
  evidence = [ordered]@{
    previous_staging_manifest_validation_version = "v0.6.14"
    previous_staging_manifest_validation_check = $previous.v0614Check
  ...
  safety = [ordered]@{
    ...
    creates_local_staging_gate_freeze_evidence = $true
    freezes_staging_gate = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.16-controlled-tester-package-prebuild-decision-gate-no-build-no-distribution")
...
$readme = @"
Voila v0.6.15 controlled tester package staging gate freeze no-create no-copy no-build no-distribution
...
Write-Host "STAGING GATE FREEZE ROOT: $freezeRoot"
Write-Host "WROTE: $freezePath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify staging gate freeze evidence files ==="
...
$allowedFreezeNames = @("STAGING-GATE-FREEZE-DRY-RUN.json", "STAGING-GATE-FREEZE-README.txt")
foreach ($name in $freezeFiles) {
  Assert-True ($allowedFreezeNames -contains $name) "Unexpected file in local-only staging gate freeze evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in staging gate freeze evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $freezePath) "Missing staging-gate-freeze JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing staging-gate-freeze README."

Write-Host ""
Write-Host "=== v0.6.15 staging gate freeze JSON ==="
$freezeJson
...
Write-Host ""
Write-Host "=== v0.6.15 CONTROLLED TESTER PACKAGE STAGING GATE FREEZE NO-CREATE NO-COPY NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
