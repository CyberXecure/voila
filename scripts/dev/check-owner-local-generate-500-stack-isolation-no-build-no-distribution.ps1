...
$Milestone = "v0.7.17b2-stack"
$FinalMarker = "VOILA_V0_7_17B2_OWNER_LOCAL_GENERATE_500_STACK_ISOLATION_CHECK=PASS"
$EvidenceRoot = Join-Path $env:TEMP "voila-v0.7.17b2-generate-500-stack-isolation"
$FixturePdf = Join-Path $EvidenceRoot "voila-v0.7.17b2-controlled-fixture.pdf"
$StdOutLog = Join-Path $EvidenceRoot "uvicorn.stdout.log"
$StdErrLog = Join-Path $EvidenceRoot "uvicorn.stderr.log"
$SourceContextPath = Join-Path $EvidenceRoot "source-context.txt"
$SummaryPath = Join-Path $EvidenceRoot "stack-isolation-summary.txt"
...
function Stop-PortOwner {
  ...
  if ($null -ne $owningProcessId) {
    Write-Step "Stopping process on port $Port PID $owningProcessId for clean diagnostic capture"
...
function New-ControlledFixturePdf {
  ...
  $pdfLines = @(
    ...
    "/F1 18 Tf",
    "72 720 Td",
    "(Voila v0.7.17b2 stack isolation fixture) Tj",
    "0 -32 Td",
    "/F1 12 Tf",
    "(Synthetic non-confidential local diagnostic PDF.) Tj",
    "0 -22 Td",
    "(Use only for exactly one upload and one generate attempt.) Tj",
    "0 -22 Td",
...
function Write-SourceContext {
  $webPath = Join-Path (Get-Location).Path "services/api/web_app.py"
  if (-not (Test-Path -LiteralPath $webPath)) {
    Set-Content -LiteralPath $SourceContextPath -Value "services/api/web_app.py not found" -Encoding utf8NoBOM
    return
  }

  $lines = Get-Content -LiteralPath $webPath
  $matchLineIndexes = New-Object System.Collections.Generic.List[int]

  for ($i = 0; $i -lt $lines.Count; $i++) {
    if (
      $lines[$i] -match 'def generate\(' -or
      $lines[$i] -match '@app\.post\("/generate"' -or
      $lines[$i] -match 'generate_for_pdf' -or
      $lines[$i] -match 'def .*pdf' -or
      $lines[$i] -match 'def .*course'
    ) {
      $matchLineIndexes.Add([int]$i)
    }
  }

  $out = New-Object System.Collections.Generic.List[string]
  $out.Add("Voila v0.7.17b2 source context")
  $out.Add("file=services/api/web_app.py")
  $out.Add("")

  foreach ($idx in ($matchLineIndexes | Sort-Object -Unique)) {
    $start = [Math]::Max(0, [int]$idx - 8)
    $end = [Math]::Min($lines.Count - 1, [int]$idx + 18)
    $out.Add("----- context around line $([int]$idx + 1) -----")
  ...
  Set-Content -LiteralPath $SourceContextPath -Value $out -Encoding utf8NoBOM
  Write-Host "source_context_path=$SourceContextPath"
}
...
function Write-StackIsolationSummary {
  ...
  if (Test-Path -LiteralPath $StdErrLog) {
    ...
    $stderrTail = ($stderr | Select-Object -Last 180) -join "`n"

    $postGenerateIndexes = New-Object System.Collections.Generic.List[int]
    for ($i = 0; $i -lt $stderr.Count; $i++) {
      if ($stderr[$i] -match 'POST /generate' -or $stderr[$i] -match 'generate\(' -or $stderr[$i] -match 'generate_for_pdf' -or $stderr[$i] -match 'Traceback') {
        $postGenerateIndexes.Add([int]$i)
      }
    }

    if ($postGenerateIndexes.Count -gt 0) {
      $first = [Math]::Max(0, [int]($postGenerateIndexes | Select-Object -First 1) - 20)
      $last = [Math]::Min($stderr.Count - 1, [int]($postGenerateIndexes | Select-Object -Last 1) + 80)
      $generateStack = ($stderr[$first..$last]) -join "`n"
    }
  }

  if (Test-Path -LiteralPath $StdOutLog) {
    $stdout = Get-Content -LiteralPath $StdOutLog -ErrorAction SilentlyContinue
    $stdoutTail = ($stdout | Select-Object -Last 120) -join "`n"
  }

  $summary = @"
Voila v0.7.17b2 /generate 500 stack isolation
...
--- generate-focused stack excerpt ---
$generateStack
...
"@
  ...
  Write-Host "stack_isolation_summary_path=$SummaryPath"
}

Write-Host ""
Write-Host "=== Voila v0.7.17b2 owner-local /generate 500 stack isolation ==="
Write-Host ""

Write-Step "Policy guard: diagnostic only; no fix; no delete/reset/cleanup"
...
Write-Host "source_context_extraction=permitted"
Write-Host "source_fix_applied=false"
...
try {
  ...
  Write-Host ""
  Write-Host "Manual stack isolation rule:"
  Write-Host "- Use only this synthetic PDF:"
  Write-Host "  $FixturePdf"
  Write-Host "- Upload exactly this file once."
  Write-Host "- Run generate exactly once."
  Write-Host "- Do not click delete, reset, cleanup, export, build, zip, delivery, or distribution."
  ...
  if ($manualResult -eq "PROMPT") {
    $manualResult = Read-Host "Stack isolation result"
  }

  if ($manualResult -ne "REPRODUCED_500" -and $manualResult -ne "NO_500") {
    throw "Invalid stack isolation result. Expected REPRODUCED_500 or NO_500."
  }

  Write-Host "manual_stack_isolation_result=$manualResult"
  Write-Host "generate_500_reproduced=$($manualResult -eq 'REPRODUCED_500')"
  Write-Host "generate_attempt_count=1"
  Write-Host "delete_reset_cleanup_performed=false"

  Write-StackIsolationSummary -ManualResult $manualResult
