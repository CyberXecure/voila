...
param(
  ...
  [switch]$OpenBrowser,
  [ValidateSet("PROMPT","PASS","FAIL")]
  [string]$Result = "PROMPT"
)

$ErrorActionPreference = "Stop"

$Milestone = "v0.7.17c-fix"
$FinalMarker = "VOILA_V0_7_17C_OWNER_LOCAL_GENERATE_ENTRYPOINT_FIX_CHECK=PASS"
$EvidenceRoot = Join-Path $env:TEMP "voila-v0.7.17c-generate-entrypoint-fix"
$FixturePdf = Join-Path $EvidenceRoot "voila-v0.7.17c-controlled-fixture.pdf"
$StdOutLog = Join-Path $EvidenceRoot "uvicorn.stdout.log"
$StdErrLog = Join-Path $EvidenceRoot "uvicorn.stderr.log"
$SummaryPath = Join-Path $EvidenceRoot "generate-entrypoint-fix-summary.txt"
...
function Stop-PortOwner {
  ...
  if ($null -ne $owningProcessId) {
    Write-Step "Stopping process on port $Port PID $owningProcessId for fix validation log capture"
...
function New-ControlledFixturePdf {
  ...
  $pdfLines = @(
    ...
    "/F1 18 Tf",
    "72 720 Td",
    "(Voila v0.7.17c generate entrypoint fixture) Tj",
    "0 -32 Td",
    "/F1 12 Tf",
    "(Synthetic non-confidential local fix validation PDF.) Tj",
    "0 -22 Td",
    "(Use only to validate generate_for_pdf restoration.) Tj",
    "0 -22 Td",
...
function Start-WithCapturedUvicorn {
  ...
  Write-Host "validation_original_uvicorn_pid=$uvicornProcessId"
  Write-Host "validation_original_uvicorn_cmd=$cmdLine"
...
function Stop-ValidationRuntime {
...
function Write-FixSummary {
  ...
  if (Test-Path -LiteralPath $StdOutLog) {
    $stdoutTail = (Get-Content -LiteralPath $StdOutLog -Tail 140 -ErrorAction SilentlyContinue) -join "`n"
  }

  $summary = @"
Voila v0.7.17c generate entrypoint fix validation

fix_validation_completed=true
manual_result=$ManualResult
generate_entrypoint_restored=$($ManualResult -eq "PASS")
base_url=$BaseUrl
...
"@
  ...
  Write-Host "fix_summary_path=$SummaryPath"
}

Write-Host ""
Write-Host "=== Voila v0.7.17c owner-local generate entrypoint fix validation ==="
Write-Host ""

Write-Step "Policy guard: generate entrypoint fix validation only; controlled synthetic fixture; no build/ZIP/distribution"
...
Write-Host "generate_for_pdf_defined=expected"
...
Write-Host "behavior_changes=limited_to_restoring_existing_generate_entrypoint"
...
try {
  ...
  Write-Host ""
  Write-Host "Manual fix validation rule:"
  ...
  Write-Host "- PASS only if Internal Server Error / 500 no longer appears and a visible generated result is created."
  Write-Host "- FAIL if /generate still shows 500 or any other blocking error."
  Write-Host "- Do not click delete, reset, cleanup, export, build, zip, delivery, or distribution."
  Write-Host ""

  $manualResult = $Result
  if ($manualResult -eq "PROMPT") {
    $manualResult = Read-Host "Fix validation result"
  }

  if ($manualResult -ne "PASS" -and $manualResult -ne "FAIL") {
    throw "Invalid fix validation result. Expected PASS or FAIL."
  }

  Write-Host "manual_fix_validation_result=$manualResult"
  Write-Host "generate_entrypoint_restored=$($manualResult -eq 'PASS')"
  Write-Host "delete_reset_cleanup_performed=false"

  Write-FixSummary -ManualResult $manualResult

  if ($manualResult -ne "PASS") {
    throw "Fix validation did not pass. No commit should be created."
  }

  Write-Host $FinalMarker
} finally {
  Stop-ValidationRuntime
