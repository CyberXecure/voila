...
Write-Host "=== v0.6.11 controlled tester package staging layout plan no-create no-copy no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-staging-layout-plan-no-create-no-copy-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-staging-layout-plan-no-create-no-copy-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$layoutRoot = "D:\dev\release-assets\voila\v0.6.11-controlled-tester-package-staging-layout-plan-no-create-no-copy-no-build-no-distribution"
$layoutPath = Join-Path $layoutRoot "STAGING-LAYOUT-PLAN-DRY-RUN.json"
$readmePath = Join-Path $layoutRoot "STAGING-LAYOUT-PLAN-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.11 file: $path"
  Write-Host $path
}
...
$v0610DocText = Get-Content -LiteralPath $previous.v0610Doc -Raw
...
Assert-True ($v0610DocText -match "v0\.6\.11-controlled-tester-package-staging-layout-plan-no-create-no-copy-no-build-no-distribution") "v0.6.10 allowed next marker missing."

Assert-True ($v0610Text -match "staging_readiness_approved_now.*false") "v0.6.10 must keep staging readiness approval blocked."
Assert-True ($v0610Text -match "staging_tree_created.*false") "v0.6.10 must keep staging tree creation blocked."
Assert-True ($v0610Text -match "staging_layout_created.*false") "v0.6.10 must keep staging layout creation blocked."
Assert-True ($v0610Text -match "copy_allowed_now.*false") "v0.6.10 must keep copy allowed blocked."
Assert-True ($v0610Text -match "copy_executed_now.*false") "v0.6.10 must keep copy execution blocked."
Assert-True ($v0610Text -match "copies_runtime_files.*false") "v0.6.10 must keep runtime copy blocked."
Assert-True ($v0610Text -match "build_allowed.*false") "v0.6.10 must keep build blocked."
Assert-True ($v0610Text -match "distribution_allowed.*false") "v0.6.10 must keep distribution blocked."
Assert-True ($v0610Text -match "uploads_to_onedrive.*false") "v0.6.10 must keep OneDrive upload blocked."
Assert-True ($v0610Text -match "publishes_github_release.*false") "v0.6.10 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.11 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.11 staging-layout-plan milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records staging layout intent only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "staging_layout_plan_approved_now.*false") "Check must keep staging_layout_plan_approved_now false."
...
Write-Host ""
Write-Host "=== Create local-only staging layout plan evidence directory ==="
New-Item -ItemType Directory -Force -Path $layoutRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$layoutFull = Resolve-FullPathSafe $layoutRoot
Assert-True (-not ($layoutFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Staging layout plan root must be outside repository."

$layoutEntries = @(
  [ordered]@{ layout_path = "app/"; purpose = "future application files placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ layout_path = "runtime/"; purpose = "future local runtime dependencies placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ layout_path = "scripts/"; purpose = "future approved launcher scripts placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ layout_path = "docs/"; purpose = "future tester documentation placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ layout_path = "legal/"; purpose = "future legal/license docs placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ layout_path = "validation/"; purpose = "future local validation report placeholder"; planned_only = $true; layout_created_now = $false; staging_layout_created = $false; staging_tree_created = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false }
)

$sourceMappings = @(
  [ordered]@{ source_path = "services"; planned_layout_path = "app/services"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ source_path = "services/api"; planned_layout_path = "app/services/api"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ source_path = "scripts/dev"; planned_layout_path = "scripts/dev"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ source_path = "docs"; planned_layout_path = "docs"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ source_path = "LICENSE.txt"; planned_layout_path = "legal/LICENSE.txt"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false },
  [ordered]@{ source_path = "scripts/dev/check-controlled-tester-package-build-plan-no-build-no-distribution.ps1"; planned_layout_path = "validation/check-controlled-tester-package-build-plan-no-build-no-distribution.ps1"; planned_only = $true; layout_created_now = $false; copy_allowed_now = $false; copy_executed_now = $false; copied_now = $false; approved_for_package_now = $false }
)

$layoutPlan = [ordered]@{
  schema_version = "1"
  staging_layout_plan_version = "v0.6.11"
  staging_layout_plan_type = "controlled_tester_package_staging_layout_plan_no_create_no_copy_no_build_no_distribution"
  status = "pass"
  staging_layout_plan = [ordered]@{
    owner_controlled_staging_layout_plan_created = $true
    layout_plan_root = $layoutRoot
    layout_plan_root_outside_repo = $true
    layout_plan_json_created = $true
    readme_created = $true
    staging_layout_plan_approved_now = $false
    ...
    next_step_policy = "STOP_OR_SEPARATE_STAGING_LAYOUT_VALIDATION_DRY_RUN_MILESTONE_ONLY"
  }
  planned_layout_entries = $layoutEntries
  planned_source_mappings = $sourceMappings
  evidence = [ordered]@{
    previous_staging_readiness_version = "v0.6.10"
    previous_staging_readiness_check = $previous.v0610Check
  ...
  gates = [ordered]@{
    ...
    staging_layout_plan_only = $true
    layout_plan_root_outside_repo = $true
    no_staging_layout_plan_approval_now = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_staging_layout_plan_evidence = $true
    approves_staging_layout_plan = $false
  ...
  allowed_next_milestones = @("STOP", "v0.6.12-controlled-tester-package-staging-layout-validation-dry-run-no-create-no-copy-no-build-no-distribution")
...
$layoutJson = $layoutPlan | ConvertTo-Json -Depth 10
[System.IO.File]::WriteAllText($layoutPath, ($layoutJson + "`n"), [System.Text.UTF8Encoding]::new($false))

$readme = @"
Voila v0.6.11 controlled tester package staging layout plan no-create no-copy no-build no-distribution
...
Write-Host "STAGING LAYOUT PLAN ROOT: $layoutRoot"
Write-Host "WROTE: $layoutPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify staging layout plan evidence files ==="
$layoutFiles = @(Get-ChildItem -LiteralPath $layoutRoot -File | Select-Object -ExpandProperty Name | Sort-Object)
foreach ($name in $layoutFiles) { Write-Host $name }

$allowedLayoutNames = @("STAGING-LAYOUT-PLAN-DRY-RUN.json", "STAGING-LAYOUT-PLAN-README.txt")
foreach ($name in $layoutFiles) {
  Assert-True ($allowedLayoutNames -contains $name) "Unexpected file in local-only staging layout plan evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in staging layout plan evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $layoutPath) "Missing staging-layout-plan JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing staging-layout-plan README."

Write-Host ""
Write-Host "=== v0.6.11 staging layout plan JSON ==="
$layoutJson
...
Write-Host ""
Write-Host "=== v0.6.11 CONTROLLED TESTER PACKAGE STAGING LAYOUT PLAN NO-CREATE NO-COPY NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
