from pathlib import Path
import hashlib
import json
import subprocess
import urllib.parse
import urllib.request
import urllib.error

root = Path(".").resolve()
course_id = "03-pag-30-34-vectori-trigonometrie"
pdf_name = course_id + ".pdf"
out_dir = root / "data" / "output" / course_id

doc_path = root / "docs" / "dev" / "ui-polish-readability-pass-no-build-no-zip-no-share-no-delivery.md"
web_path = root / "services" / "api" / "web_app.py"

watch_paths = [
    out_dir / "explanation_drafts.preview.json",
    out_dir / "manual_learning_evidence.json",
    out_dir / "manual_study_items.preview.json",
    out_dir / "study_items.preview.json",
]

def fail(message: str):
    raise SystemExit(message)

def sha_or_missing(path: Path) -> str:
    if not path.exists():
        return "MISSING"
    return hashlib.sha256(path.read_bytes()).hexdigest()

def fetch(url: str):
    try:
        with urllib.request.urlopen(url, timeout=20) as response:
            return response.status, response.read().decode("utf-8", errors="replace")
    except urllib.error.HTTPError as exc:
        return exc.code, exc.read().decode("utf-8", errors="replace")

for path in [doc_path, web_path]:
    if not path.exists():
        fail("FAILED_V0859_REQUIRED_FILE_MISSING=" + str(path))

doc = doc_path.read_text(encoding="utf-8", errors="replace")
web = web_path.read_text(encoding="utf-8", errors="replace")

for term in [
    "v0.8.59 UI polish/readability pass",
    "It does not change `services/api/web_app.py`.",
    "Course Tools",
    "Revizuire document",
    "Text detectat",
    "Corecturi sugerate",
    "Formule și imagini",
    "Explicație prietenoasă",
    "Draft explicație",
    "Study curat",
    "This pass does not submit forms.",
    "It does not call POST endpoints.",
    "It does not create a new explanation draft.",
    "It does not write manual evidence.",
    "It does not create Study cards.",
    "It does not write Progress.",
    "No build.",
    "No ZIP.",
    "No share.",
    "No delivery.",
]:
    if term not in doc:
        fail("FAILED_V0859_DOC_TERM_MISSING=" + term)

for marker in [
    "VOILA_V0_8_50_REVIEW_DOCUMENT_SHELL_READ_ONLY_FIRST_SLICE_START",
    "VOILA_V0_8_51_COURSE_TOOLS_LINK_TO_REVIEW_DOCUMENT_SHELL_START",
    "VOILA_V0_8_52_TEXT_DETECTED_READ_ONLY_QUEUE_START",
    "VOILA_V0_8_53_CORRECTIONS_SUGGESTED_READ_ONLY_QUEUE_START",
    "VOILA_V0_8_54_FORMULAS_IMAGES_READ_ONLY_QUEUE_START",
    "VOILA_V0_8_55_FRIENDLY_EXPLANATION_FORM_READ_ONLY_STATIC_DRAFT_SHELL_START",
    "VOILA_V0_8_56_SAFE_LOCAL_SAVE_FOR_EXPLANATION_DRAFTS_START",
    "VOILA_V0_8_57_CLEAN_STUDY_READ_ONLY_PREVIEW_FROM_EXPLANATION_DRAFTS_START",
]:
    if marker not in web:
        fail("FAILED_V0859_REQUIRED_CHAIN_MARKER_MISSING=" + marker)

changed = subprocess.check_output(
    ["git", "status", "--porcelain", "-uall"],
    text=True,
    encoding="utf-8",
    errors="replace",
).splitlines()

allowed = {
    "docs/dev/ui-polish-readability-pass-no-build-no-zip-no-share-no-delivery.md",
    "scripts/dev/check-ui-polish-readability-pass-no-build-no-zip-no-share-no-delivery.py",
    "scripts/dev/check-ui-polish-readability-pass-no-build-no-zip-no-share-no-delivery.ps1",
}

for line in changed:
    if not line.strip():
        continue
    path = line[3:].replace("\\", "/")
    if path not in allowed:
        fail("FAILED_V0859_UNEXPECTED_GIT_STATUS_PATH=" + line)

before = {str(path): sha_or_missing(path) for path in watch_paths}

health_status, _ = fetch("http://127.0.0.1:8787/health")
if health_status != 200:
    fail("FAILED_V0859_HEALTH_STATUS=" + str(health_status))

course_tools_url = "http://127.0.0.1:8787/course-tools?pdf=" + urllib.parse.quote(pdf_name)
review_url = "http://127.0.0.1:8787/review-document?pdf=" + urllib.parse.quote(pdf_name)
draft_form_url = review_url + "&draft=1"
clean_preview_url = "http://127.0.0.1:8787/study-clean-preview?pdf=" + urllib.parse.quote(pdf_name)

course_status, course_html = fetch(course_tools_url)
review_status, review_html = fetch(review_url)
draft_status, draft_html = fetch(draft_form_url)
clean_status, clean_html = fetch(clean_preview_url)

if course_status != 200:
    fail("FAILED_V0859_COURSE_TOOLS_STATUS=" + str(course_status))
if review_status != 200:
    fail("FAILED_V0859_REVIEW_STATUS=" + str(review_status))
if draft_status != 200:
    fail("FAILED_V0859_DRAFT_FORM_STATUS=" + str(draft_status))
if clean_status != 200:
    fail("FAILED_V0859_CLEAN_PREVIEW_STATUS=" + str(clean_status))

course_required = [
    "Învață pentru examen din acest document",
    "Revizuiește documentul",
    "Alege noțiuni importante",
    "Creează material de învățare",
    "Învață acum",
    "Exersează pentru examen",
    "Deschide Revizuire document",
]

for term in course_required:
    if term not in course_html:
        fail("FAILED_V0859_COURSE_READABILITY_TERM_MISSING=" + term)

review_required = [
    "Revizuire document",
    "Text detectat",
    "Corecturi sugerate",
    "Formule și imagini",
    "Explicație prietenoasă",
    "Draft explicație",
    "Creează draft local",
    "Study curat",
    "Deschide previzualizare Study curat",
    "Doar citire",
]

for term in review_required:
    if term not in review_html:
        fail("FAILED_V0859_REVIEW_READABILITY_TERM_MISSING=" + term)

draft_required = [
    "Creează draft local",
    "Titlu scurt",
    "Ce este asta?",
    "Text / zonă verificată",
    "Explicație pe înțeles",
    "De ce este important?",
    "Sursa: pagina X",
    "Limba lecției",
    "Gata pentru studiu",
    "Salvează draft local",
]

for term in draft_required:
    if term not in draft_html:
        fail("FAILED_V0859_DRAFT_READABILITY_TERM_MISSING=" + term)

draft_lower = draft_html.lower()
for term in [
    "creează study cards",
    "scrie progress",
    "face delivery",
]:
    if term not in draft_lower:
        fail("FAILED_V0859_DRAFT_READABILITY_TERM_MISSING=" + term)

clean_required = [
    "study-clean-preview-route",
    "Study curat",
    "previzualizare",
    "Read-only",
    "nu creează carduri reale",
    "nu scrie Progress",
    "Înapoi la Revizuire document",
]

for term in clean_required:
    if term not in clean_html:
        fail("FAILED_V0859_CLEAN_READABILITY_TERM_MISSING=" + term)

if not ("study-clean-preview-card" in clean_html or "study-clean-preview-empty-state" in clean_html):
    fail("FAILED_V0859_CLEAN_CARD_OR_EMPTY_STATE_MISSING")

if "friendly-explanation-draft-save-result" in draft_html or "friendly-explanation-draft-save-result" in clean_html:
    fail("FAILED_V0859_POST_RESULT_RENDERED_WITHOUT_POST")

if "<form" in clean_html.lower():
    fail("FAILED_V0859_CLEAN_PREVIEW_FORM_PRESENT")

for html_name, html in [
    ("review", review_html),
    ("draft", draft_html),
    ("clean", clean_html),
]:
    main_part = html.split("Diagnostic tehnic", 1)[0]
    lowered = main_part.lower()
    for forbidden in [
        "bbox",
        "crop_path",
        "visual_evidence_id",
        "source_evidence_id",
        "manual_study_item_id",
        "delivery_performed",
        "build_performed",
        "zip_created",
    ]:
        if forbidden in lowered:
            fail("FAILED_V0859_TECHNICAL_LABEL_VISIBLE_IN_MAIN_SURFACE=" + html_name + ":" + forbidden)

after = {str(path): sha_or_missing(path) for path in watch_paths}

changed_artifacts = [
    path for path in watch_paths
    if after[str(path)] != before[str(path)]
]
if changed_artifacts:
    fail("FAILED_V0859_READABILITY_PASS_CHANGED_ARTIFACTS=" + ",".join(str(path) for path in changed_artifacts))

evidence_dir = Path(r"D:\dev\tester-runs\v0859-ui-polish-readability-pass-no-build-no-zip-no-share-no-delivery")
evidence_dir.mkdir(parents=True, exist_ok=True)

summary = {
    "VOILA_V0_8_59_UI_POLISH_READABILITY_PASS_CHECK": "PASS",
    "implementation_performed": False,
    "ui_readability_pass_performed": True,
    "course_tools_status": course_status,
    "review_document_status": review_status,
    "draft_form_get_status": draft_status,
    "clean_preview_status": clean_status,
    "course_tools_workflow_copy_visible": True,
    "review_document_steps_visible": True,
    "friendly_labels_visible": True,
    "draft_form_labels_visible": True,
    "clean_preview_read_only_copy_visible": True,
    "clean_preview_card_or_empty_state_visible": True,
    "diagnostic_boundary_respected": True,
    "main_surface_technical_labels_hidden": True,
    "no_post_called": True,
    "no_post_result_rendered": True,
    "read_only_pass": True,
    "explanation_drafts_unchanged": True,
    "manual_learning_evidence_unchanged": True,
    "manual_study_items_preview_unchanged": True,
    "study_items_preview_unchanged": True,
    "web_app_changed": False,
    "new_route_added": False,
    "new_post_endpoint_added": False,
    "default_study_changed": False,
    "study_cards_created": False,
    "progress_write_added": False,
    "manual_evidence_written": False,
    "ocr_started": False,
    "languagetool_started": False,
    "formula_ocr_performed": False,
    "crop_extraction_performed": False,
    "build_performed": False,
    "package_rebuild_performed": False,
    "new_zip_created": False,
    "share_created": False,
    "delivery_performed": False,
    "public_release_created": False,
    "UI_POLISH_READABILITY_PASS": "PASS_OWNER_LOCAL_READ_ONLY_NO_BUILD_NO_ZIP_NO_SHARE_NO_DELIVERY",
    "PACKAGE_READINESS": "BLOCKED_PENDING_UI_IMPLEMENTATION_AND_RETEST",
    "POLICY": "ui_polish_readability_pass_no_build_no_zip_no_share_no_delivery",
    "RECOMMENDED_NEXT": "v0.8.60-owner-local-package-rebuild-preflight-after-owner-approval-no-build-no-zip-no-share-no-delivery",
}

out_json = evidence_dir / "V0.8.59-UI-POLISH-READABILITY-PASS-CHECK.json"
out_md = evidence_dir / "V0.8.59-UI-POLISH-READABILITY-PASS-CHECK.md"

out_json.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
out_md.write_text(
    "# v0.8.59 UI polish/readability pass\n\n"
    + "\n".join(f"- {key}: {value}" for key, value in summary.items())
    + "\n",
    encoding="utf-8",
)

for key, value in summary.items():
    print(f"{key}={value}")

print("EVIDENCE_JSON=" + str(out_json))
print("EVIDENCE_MD=" + str(out_md))
