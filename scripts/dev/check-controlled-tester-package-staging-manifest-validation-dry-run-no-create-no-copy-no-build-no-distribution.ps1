...
Write-Host "=== v0.6.14 controlled tester package staging manifest validation dry-run no-create no-copy no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-staging-manifest-validation-dry-run-no-create-no-copy-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-staging-manifest-validation-dry-run-no-create-no-copy-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$validationRoot = "D:\dev\release-assets\voila\v0.6.14-controlled-tester-package-staging-manifest-validation-dry-run-no-create-no-copy-no-build-no-distribution"
$validationPath = Join-Path $validationRoot "STAGING-MANIFEST-VALIDATION-DRY-RUN.json"
$readmePath = Join-Path $validationRoot "STAGING-MANIFEST-VALIDATION-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.14 file: $path"
  Write-Host $path
}
...
$v0613DocText = Get-Content -LiteralPath $previous.v0613Doc -Raw
...
Assert-True ($v0613DocText -match "v0\.6\.14-controlled-tester-package-staging-manifest-validation-dry-run-no-create-no-copy-no-build-no-distribution") "v0.6.13 allowed next marker missing."

Assert-True ($v0613Text -match "staging_manifest_plan_approved_now.*false") "v0.6.13 must keep staging manifest plan approval blocked."
Assert-True ($v0613Text -match "manifest_approved_now.*false") "v0.6.13 must keep manifest approval blocked."
Assert-True ($v0613Text -match "manifest_created_now.*false") "v0.6.13 must keep manifest creation blocked."
Assert-True ($v0613Text -match "staging_manifest_created.*false") "v0.6.13 must keep staging manifest creation blocked."
Assert-True ($v0613Text -match "staging_layout_created.*false") "v0.6.13 must keep staging layout creation blocked."
Assert-True ($v0613Text -match "staging_tree_created.*false") "v0.6.13 must keep staging tree creation blocked."
Assert-True ($v0613Text -match "staging_root_created.*false") "v0.6.13 must keep staging root creation blocked."
Assert-True ($v0613Text -match "copy_allowed_now.*false") "v0.6.13 must keep copy allowed blocked."
Assert-True ($v0613Text -match "copy_executed_now.*false") "v0.6.13 must keep copy execution blocked."
Assert-True ($v0613Text -match "copies_runtime_files.*false") "v0.6.13 must keep runtime copy blocked."
Assert-True ($v0613Text -match "build_allowed.*false") "v0.6.13 must keep build blocked."
Assert-True ($v0613Text -match "distribution_allowed.*false") "v0.6.13 must keep distribution blocked."
Assert-True ($v0613Text -match "uploads_to_onedrive.*false") "v0.6.13 must keep OneDrive upload blocked."
Assert-True ($v0613Text -match "publishes_github_release.*false") "v0.6.13 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.14 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.14 staging-manifest-validation milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records staging manifest validation only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "staging_manifest_validation_approved_now.*false") "Check must keep staging_manifest_validation_approved_now false."
Assert-True ($checkText -match "manifest_validation_approved_now.*false") "Check must keep manifest_validation_approved_now false."
Assert-True ($checkText -match "staging_manifest_enforced_now.*false") "Check must keep staging_manifest_enforced_now false."
Assert-True ($checkText -match "manifest_approved_now.*false") "Check must keep manifest_approved_now false."
Assert-True ($checkText -match "manifest_created_now.*false") "Check must keep manifest_created_now false."
Assert-True ($checkText -match "staging_manifest_created.*false") "Check must keep staging_manifest_created false."
...
Write-Host ""
Write-Host "=== Create local-only staging manifest validation evidence directory ==="
New-Item -ItemType Directory -Force -Path $validationRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$validationFull = Resolve-FullPathSafe $validationRoot
Assert-True (-not ($validationFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Staging manifest validation root must be outside repository."
...
foreach ($section in $manifestSections) {
  $validatedSections += [ordered]@{
    section_name = $section
    validation_checked = $true
    section_name_valid = [bool](Test-ManifestSectionName $section)
    planned_only = $true
    staging_manifest_validation_approved_now = $false
    manifest_validation_approved_now = $false
    staging_manifest_enforced_now = $false
    manifest_approved_now = $false
    manifest_created_now = $false
    staging_manifest_created = $false
    staging_layout_created = $false
...
$plannedManifestEntries = @(
  [ordered]@{ section_name = "metadata"; entry_name = "schema_version"; planned_value = "future-only" },
  [ordered]@{ section_name = "metadata"; entry_name = "package_candidate_version"; planned_value = "future-only" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "app"; planned_value = "app/" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "runtime"; planned_value = "runtime/" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "scripts"; planned_value = "scripts/" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "docs"; planned_value = "docs/" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "legal"; planned_value = "legal/" },
  [ordered]@{ section_name = "layout_entries"; entry_name = "validation"; planned_value = "validation/" },
  [ordered]@{ section_name = "source_mappings"; entry_name = "services"; planned_value = "services > app/services" },
  [ordered]@{ section_name = "source_mappings"; entry_name = "services_api"; planned_value = "services/api > app/services/api" },
  [ordered]@{ section_name = "source_mappings"; entry_name = "scripts_dev"; planned_value = "scripts/dev > scripts/dev" },
  [ordered]@{ section_name = "source_mappings"; entry_name = "docs"; planned_value = "docs > docs" },
  [ordered]@{ section_name = "source_mappings"; entry_name = "license"; planned_value = "LICENSE.txt > legal/LICENSE.txt" },
  [ordered]@{ section_name = "validation"; entry_name = "local_check"; planned_value = "check-controlled-tester-package-build-plan-no-build-no-distribution.ps1" },
  [ordered]@{ section_name = "safety_gates"; entry_name = "no_copy"; planned_value = "copy_allowed_now false" },
  [ordered]@{ section_name = "safety_gates"; entry_name = "no_build"; planned_value = "build_allowed false" },
  [ordered]@{ section_name = "blocked_actions"; entry_name = "distribution"; planned_value = "distribution_allowed false" }
)

$validatedEntries = @()
foreach ($entry in $plannedManifestEntries) {
  $validatedEntries += [ordered]@{
    section_name = $entry.section_name
    entry_name = $entry.entry_name
    planned_value = $entry.planned_value
    validation_checked = $true
    section_name_valid = [bool](Test-ManifestSectionName $entry.section_name)
    entry_name_valid = [bool](Test-ManifestSectionName $entry.entry_name)
    entry_value_valid = [bool](Test-ManifestEntryString $entry.planned_value)
    planned_only = $true
    staging_manifest_validation_approved_now = $false
    manifest_validation_approved_now = $false
    staging_manifest_enforced_now = $false
...
$validation = [ordered]@{
  schema_version = "1"
  staging_manifest_validation_version = "v0.6.14"
  staging_manifest_validation_type = "controlled_tester_package_staging_manifest_validation_dry_run_no_create_no_copy_no_build_no_distribution"
  status = "pass"
  staging_manifest_validation = [ordered]@{
    ...
    next_step_policy = "STOP_OR_SEPARATE_STAGING_GATE_FREEZE_MILESTONE_ONLY"
  }
  validated_manifest_sections = $validatedSections
  validated_manifest_entries = $validatedEntries
  evidence = [ordered]@{
    previous_staging_manifest_plan_version = "v0.6.13"
    previous_staging_manifest_plan_check = $previous.v0613Check
  ...
  gates = [ordered]@{
    ...
    staging_manifest_validation_dry_run_only = $true
    validation_root_outside_repo = $true
    validation_checked = $true
    no_staging_manifest_validation_approval_now = $true
    no_manifest_validation_approval_now = $true
    no_staging_manifest_enforcement_now = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_staging_manifest_validation_evidence = $true
    approves_staging_manifest_validation = $false
    approves_manifest_validation = $false
  ...
  allowed_next_milestones = @("STOP", "v0.6.15-controlled-tester-package-staging-gate-freeze-no-create-no-copy-no-build-no-distribution")
...
$readme = @"
Voila v0.6.14 controlled tester package staging manifest validation dry-run no-create no-copy no-build no-distribution
...
$allowedValidationNames = @("STAGING-MANIFEST-VALIDATION-DRY-RUN.json", "STAGING-MANIFEST-VALIDATION-README.txt")
foreach ($name in $validationFiles) {
  Assert-True ($allowedValidationNames -contains $name) "Unexpected file in local-only staging manifest validation evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in staging manifest validation evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $validationPath) "Missing staging-manifest-validation JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing staging-manifest-validation README."

Write-Host ""
Write-Host "=== v0.6.14 staging manifest validation JSON ==="
$validationJson
...
Write-Host ""
Write-Host "=== v0.6.14 CONTROLLED TESTER PACKAGE STAGING MANIFEST VALIDATION DRY-RUN NO-CREATE NO-COPY NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
