...
Set-Content -Path $pdfPath -Value "%PDF-1.4`n% v0.7.45 owner visual smoke placeholder`n" -Encoding ASCII
...
targets = [
    (out / "figures_hybrid" / "crops" / "figure_001.png", "Existing hybrid crop image"),
    (out / "ocr" / "page_images" / "page_0001.png", "OCR Math sidecar page image"),
]
...
for path, label in targets:
    ...
    draw.text((50, 95), label, fill=(35, 30, 25))
    draw.text((50, 135), "v0.7.45 visual smoke", fill=(80, 70, 60))
    image.save(path)
    print(f"WROTE={path}")
"@

$pngWriterPath = Join-Path $env:TEMP "voila-v0.7.45-write-valid-pngs.py"
...
$realManifest = [ordered]@{
  ...
  figure_crops = @(
    [ordered]@{
      number = "1"
      caption = "Existing hybrid figure crop for v0.7.45 visual smoke"
      pdf_page = 1
...
$sidecar = [ordered]@{
  ...
  figure_crops = @(
    [ordered]@{
      ...
      caption = "OCR Math visual fallback candidate: formula / symbol may need visual crop"
      pdf_page = 1
