...
Write-Host "=== v0.6.23 controlled tester package release approval freeze no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-release-approval-freeze-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-release-approval-freeze-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$freezeRoot = "D:\dev\release-assets\voila\v0.6.23-controlled-tester-package-release-approval-freeze-no-build-no-distribution"
$freezePath = Join-Path $freezeRoot "RELEASE-APPROVAL-FREEZE-DRY-RUN.json"
$readmePath = Join-Path $freezeRoot "RELEASE-APPROVAL-FREEZE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.23 file: $path"
  Write-Host $path
}
...
$v0622DocText = Get-Content -LiteralPath $previous.v0622Doc -Raw
...
Assert-True ($v0622DocText -match "v0\.6\.23-controlled-tester-package-release-approval-freeze-no-build-no-distribution") "v0.6.22 allowed next marker missing."

Assert-True ($v0622Text -match "release_gate_validation_created.*true") "v0.6.22 must have release gate validation."
Assert-True ($v0622Text -match "release_gate_validation_checked.*true") "v0.6.22 must check release gate validation."
Assert-True ($v0622Text -match "release_gate_validation_dry_run_only.*true") "v0.6.22 must keep release gate validation dry-run only."
Assert-True ($v0622Text -match "release_gate_validation_approved_now.*false") "v0.6.22 must keep release gate validation approval blocked."
Assert-True ($v0622Text -match "release_gate_approved_now.*false") "v0.6.22 must keep release gate approval blocked."
Assert-True ($v0622Text -match "release_gate_enforced_now.*false") "v0.6.22 must keep release gate enforcement blocked."
Assert-True ($v0622Text -match "public_release_allowed_now.*false") "v0.6.22 must keep public release blocked."
Assert-True ($v0622Text -match "tester_delivery_allowed_now.*false") "v0.6.22 must keep tester delivery blocked."
Assert-True ($v0622Text -match "distribution_allowed.*false") "v0.6.22 must keep distribution blocked."
Assert-True ($v0622Text -match "onedrive_upload_allowed_now.*false") "v0.6.22 must keep OneDrive upload blocked."
Assert-True ($v0622Text -match "github_release_allowed_now.*false") "v0.6.22 must keep GitHub release blocked."
Assert-True ($v0622Text -match "public_website_upload_allowed_now.*false") "v0.6.22 must keep public website upload blocked."
Assert-True ($v0622Text -match "build_readiness_unfreeze_allowed_now.*false") "v0.6.22 must keep build readiness unfreeze blocked."
Assert-True ($v0622Text -match "package_build_executed_now.*false") "v0.6.22 must keep package build blocked."
Assert-True ($v0622Text -match "zip_created.*false") "v0.6.22 must keep zip creation blocked."
Assert-True ($v0622Text -match "release_archive_created.*false") "v0.6.22 must keep release archive blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.23 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.23 release-approval-freeze milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "No live scoring",
  "release_approval_freeze_created = true",
  "release_approval_freeze_active = true",
  "release_approval_frozen = true",
  "release_gate_validation_checked = true",
  "release_gate_validation_dry_run_only = true",
  ...
  "This milestone records release approval freeze only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "release_approval_freeze_created.*true") "Check must set release_approval_freeze_created true."
Assert-True ($checkText -match "release_approval_freeze_active.*true") "Check must set release_approval_freeze_active true."
Assert-True ($checkText -match "release_approval_frozen.*true") "Check must set release_approval_frozen true."
Assert-True ($checkText -match "release_gate_validation_checked.*true") "Check must keep release_gate_validation_checked true."
Assert-True ($checkText -match "release_gate_validation_dry_run_only.*true") "Check must keep release_gate_validation_dry_run_only true."
...
Write-Host ""
Write-Host "=== Create local-only release approval freeze evidence directory ==="
...
Assert-True (-not ($freezeFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Release approval freeze root must be outside repository."

$frozenReleaseApprovalCategories = @(
  "controlled_tester_audience_approval",
  "package_artifact_approval",
  "checksum_approval",
  "license_and_beta_terms_approval",
  "known_limitations_approval",
  "onedrive_share_approval",
  "github_release_approval",
  "public_website_upload_approval",
  "tester_delivery_approval",
  "rollback_and_revocation_approval",
  "final_owner_go_no_go_approval"
)

$frozenEntries = @()
foreach ($category in $frozenReleaseApprovalCategories) {
...
$freeze = [ordered]@{
  schema_version = "1"
  release_approval_freeze_version = "v0.6.23"
  release_approval_freeze_type = "controlled_tester_package_release_approval_freeze_no_build_no_distribution"
  status = "pass"
  release_approval_freeze = [ordered]@{
    owner_controlled_release_approval_freeze_created = $true
    release_approval_freeze_created = $true
    release_approval_freeze_active = $true
    release_approval_frozen = $true
    release_gate_validation_checked = $true
    ...
    next_step_policy = "STOP_OR_SEPARATE_RELEASE_READINESS_REVIEW_DRY_RUN_NO_BUILD_MILESTONE_ONLY"
  }
  frozen_release_approval_categories = $frozenEntries
  evidence = [ordered]@{
    previous_release_gate_validation_version = "v0.6.22"
    previous_release_gate_validation_check = $previous.v0622Check
  ...
  gates = [ordered]@{
    ...
    release_approval_freeze_only = $true
    freeze_root_outside_repo = $true
    release_approval_freeze_active = $true
    release_approval_frozen = $true
    release_gate_validation_checked = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_release_approval_freeze_evidence = $true
    freezes_release_approval = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.24-controlled-tester-package-release-readiness-review-dry-run-no-build-no-distribution")
...
$readme = @"
Voila v0.6.23 controlled tester package release approval freeze no-build no-distribution
...
$allowedFreezeNames = @("RELEASE-APPROVAL-FREEZE-DRY-RUN.json", "RELEASE-APPROVAL-FREEZE-README.txt")
foreach ($name in $freezeFiles) {
  Assert-True ($allowedFreezeNames -contains $name) "Unexpected file in local-only release approval freeze evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in release approval freeze evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $freezePath) "Missing release-approval-freeze JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing release-approval-freeze README."

Write-Host ""
Write-Host "=== v0.6.23 release approval freeze JSON ==="
$freezeJson
...
Write-Host ""
Write-Host "=== v0.6.23 CONTROLLED TESTER PACKAGE RELEASE APPROVAL FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
