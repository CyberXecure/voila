...
Write-Host "=== v0.6.21 controlled tester package release gate plan no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-release-gate-plan-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-release-gate-plan-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$planRoot = "D:\dev\release-assets\voila\v0.6.21-controlled-tester-package-release-gate-plan-no-build-no-distribution"
$planPath = Join-Path $planRoot "RELEASE-GATE-PLAN-DRY-RUN.json"
$readmePath = Join-Path $planRoot "RELEASE-GATE-PLAN-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.21 file: $path"
  Write-Host $path
}
...
$v0620DocText = Get-Content -LiteralPath $previous.v0620Doc -Raw
...
Assert-True ($v0620DocText -match "v0\.6\.21-controlled-tester-package-release-gate-plan-no-build-no-distribution") "v0.6.20 allowed next marker missing."

Assert-True ($v0620Text -match "build_readiness_freeze_active.*true") "v0.6.20 must keep build readiness freeze active."
Assert-True ($v0620Text -match "build_readiness_frozen.*true") "v0.6.20 must keep build readiness frozen."
Assert-True ($v0620Text -match "build_readiness_approved_now.*false") "v0.6.20 must keep build readiness approval blocked."
Assert-True ($v0620Text -match "build_readiness_unfreeze_allowed_now.*false") "v0.6.20 must keep build readiness unfreeze blocked."
Assert-True ($v0620Text -match "build_approval_allowed_now.*false") "v0.6.20 must keep build approval blocked."
Assert-True ($v0620Text -match "build_validation_approval_allowed_now.*false") "v0.6.20 must keep build validation approval blocked."
Assert-True ($v0620Text -match "build_approved_now.*false") "v0.6.20 must keep build approved false."
Assert-True ($v0620Text -match "build_allowed_now.*false") "v0.6.20 must keep build allowed blocked."
Assert-True ($v0620Text -match "package_build_executed_now.*false") "v0.6.20 must keep package build execution blocked."
Assert-True ($v0620Text -match "zip_created.*false") "v0.6.20 must keep zip creation blocked."
Assert-True ($v0620Text -match "installer_created.*false") "v0.6.20 must keep installer creation blocked."
Assert-True ($v0620Text -match "release_archive_created.*false") "v0.6.20 must keep release archive creation blocked."
Assert-True ($v0620Text -match "distribution_allowed.*false") "v0.6.20 must keep distribution blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.21 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.21 release-gate-plan milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "This milestone records a release gate plan only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "release_gate_plan_created.*true") "Check must set release_gate_plan_created true."
Assert-True ($checkText -match "release_gate_plan_dry_run_only.*true") "Check must keep release_gate_plan_dry_run_only true."
...
Write-Host ""
Write-Host "=== Create local-only release gate plan evidence directory ==="
...
Assert-True (-not ($planFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Release gate plan root must be outside repository."
...
$plan = [ordered]@{
  schema_version = "1"
  release_gate_plan_version = "v0.6.21"
  release_gate_plan_type = "controlled_tester_package_release_gate_plan_no_build_no_distribution"
  status = "pass"
  release_gate_plan = [ordered]@{
    ...
    next_step_policy = "STOP_OR_SEPARATE_RELEASE_GATE_VALIDATION_DRY_RUN_NO_BUILD_MILESTONE_ONLY"
  }
  planned_release_gate_categories = $releaseGateEntries
  evidence = [ordered]@{
    previous_build_readiness_freeze_version = "v0.6.20"
    previous_build_readiness_freeze_check = $previous.v0620Check
  ...
  safety = [ordered]@{
    ...
    creates_local_release_gate_plan_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.22-controlled-tester-package-release-gate-validation-dry-run-no-build-no-distribution")
...
$readme = @"
Voila v0.6.21 controlled tester package release gate plan no-build no-distribution
...
$allowedPlanNames = @("RELEASE-GATE-PLAN-DRY-RUN.json", "RELEASE-GATE-PLAN-README.txt")
foreach ($name in $planFiles) {
  Assert-True ($allowedPlanNames -contains $name) "Unexpected file in local-only release gate plan evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in release gate plan evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $planPath) "Missing release-gate-plan JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing release-gate-plan README."

Write-Host ""
Write-Host "=== v0.6.21 release gate plan JSON ==="
$planJson
...
Write-Host ""
Write-Host "=== v0.6.21 CONTROLLED TESTER PACKAGE RELEASE GATE PLAN NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
