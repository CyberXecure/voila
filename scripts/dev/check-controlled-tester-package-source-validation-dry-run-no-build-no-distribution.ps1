...
Write-Host "=== v0.6.6 controlled tester package source validation dry-run no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-source-validation-dry-run-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-source-validation-dry-run-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$validationRoot = "D:\dev\release-assets\voila\v0.6.6-controlled-tester-package-source-validation-dry-run-no-build-no-distribution"
$validationPath = Join-Path $validationRoot "SOURCE-VALIDATION-DRY-RUN.json"
$readmePath = Join-Path $validationRoot "SOURCE-VALIDATION-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.6 file: $path"
  Write-Host $path
}
...
$v065DocText = Get-Content -LiteralPath $previous.v065Doc -Raw
...
Assert-True ($v065DocText -match "v0\.6\.6-controlled-tester-package-source-validation-dry-run-no-build-no-distribution") "v0.6.5 allowed next marker missing."
...
Assert-True ($v065Text -match "approved_for_package_now.*false") "v0.6.5 must keep package approval blocked."
Assert-True ($v065Text -match "copies_runtime_files.*false") "v0.6.5 must keep runtime copy blocked."
Assert-True ($v065Text -match "build_allowed.*false") "v0.6.5 must keep build blocked."
Assert-True ($v065Text -match "distribution_allowed.*false") "v0.6.5 must keep distribution blocked."
Assert-True ($v065Text -match "uploads_to_onedrive.*false") "v0.6.5 must keep OneDrive upload blocked."
Assert-True ($v065Text -match "publishes_github_release.*false") "v0.6.5 must keep GitHub release blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.6 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.6 source-validation milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "No live scoring",
  "This milestone records path existence and type only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "source_validation_approved_now.*false") "Check must keep source_validation_approved_now false."
...
Write-Host ""
Write-Host "=== Create local-only source validation evidence directory ==="
New-Item -ItemType Directory -Force -Path $validationRoot | Out-Null

$repoFull = Resolve-FullPathSafe $repoRoot
$validationFull = Resolve-FullPathSafe $validationRoot
Assert-True (-not ($validationFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Source validation root must be outside repository."
...
foreach ($candidatePath in $candidatePaths) {
  ...
  $kind = Get-PathKind $candidatePath
  $validatedCandidates += [ordered]@{
    candidate_path = $candidatePath
    exists = [bool]$exists
    path_kind = $kind
    status = "validated_for_existence_only"
    copied_now = $false
...
$validation = [ordered]@{
  schema_version = "1"
  validation_version = "v0.6.6"
  validation_type = "controlled_tester_package_source_validation_dry_run_no_build_no_distribution"
  status = "pass"
  source_validation = [ordered]@{
    ...
    next_step_policy = "STOP_OR_SEPARATE_SOURCE_ALLOWLIST_PLAN_MILESTONE_ONLY"
  }
  validated_candidate_source_paths = $validatedCandidates
  evidence = [ordered]@{
    previous_source_selection_version = "v0.6.5"
    previous_source_selection_check = $previous.v065Check
  ...
  gates = [ordered]@{
    ...
    source_validation_dry_run_only = $true
    validation_root_outside_repo = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_source_validation_evidence = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.7-controlled-tester-package-source-allowlist-plan-no-build-no-distribution")
...
$readme = @"
Voila v0.6.6 controlled tester package source validation dry-run no-build no-distribution
...
Blocked:
- runtime file copy, package source approval, source allowlist approval, package build, ZIP, EXE, MSI, installer, runnable package, release archive, release asset staging, code signing, checksum publication, public UI, tester activation, submit flow, persistence, scoring, cloud/API.
"@

[System.IO.File]::WriteAllText($readmePath, ($readme.TrimEnd() + "`n"), [System.Text.UTF8Encoding]::new($false))

Write-Host "SOURCE VALIDATION ROOT: $validationRoot"
Write-Host "WROTE: $validationPath"
Write-Host "WROTE: $readmePath"

Write-Host ""
Write-Host "=== Verify source validation evidence files ==="
...
$allowedValidationNames = @("SOURCE-VALIDATION-DRY-RUN.json", "SOURCE-VALIDATION-README.txt")
foreach ($name in $validationFiles) {
  Assert-True ($allowedValidationNames -contains $name) "Unexpected file in local-only source validation evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in source validation evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $validationPath) "Missing source-validation JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing source-validation README."

Write-Host ""
Write-Host "=== v0.6.6 source validation JSON ==="
$validationJson
...
Write-Host ""
Write-Host "=== v0.6.6 CONTROLLED TESTER PACKAGE SOURCE VALIDATION DRY-RUN NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
