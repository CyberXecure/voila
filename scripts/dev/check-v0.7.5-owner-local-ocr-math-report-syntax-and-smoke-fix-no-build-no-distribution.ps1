...
$Doc = Join-Path $RepoRoot "docs\dev\v0.7.5-owner-local-ocr-math-report-syntax-and-smoke-fix-no-build-no-distribution.md"

if (-not (Test-Path $WebApp)) {
  throw "Missing services/api/web_app.py"
}
if (-not (Test-Path $Doc)) {
  throw "Missing v0.7.5 docs file"
}
...
foreach ($marker in $RequiredWebMarkers) {
  if ($WebText -notlike "*$marker*") {
    throw "Missing repaired web marker: $marker"
  }
}

$RequiredDocMarkers = @(
  "syntax and smoke-validation repair only",
  "IndentationError",
  "v0.7.4 smoke",
  "import-move repairing",
  "safe top-level FastAPI/import anchor with import relocation",
  "no build",
  "no ZIP",
  "no delivery",
  "no distribution",
...
foreach ($marker in $RequiredDocMarkers) {
  if ($DocText -notlike "*$marker*") {
    throw "Missing v0.7.5 doc marker: $marker"
  }
}
...
if ($LASTEXITCODE -ne 0) {
  throw "Python syntax check failed for services/api/web_app.py"
}

Write-Host "== Prepare temporary local smoke fixture =="
$FixtureRoot = Join-Path ([System.IO.Path]::GetTempPath()) "voila-v0.7.5-ocr-math-report-smoke-fixture"
...
fixture_root = Path(os.environ["VOILA_V075_FIXTURE_ROOT"]).resolve()
course_id = "v0-7-5-ocr-math-smoke-course"
course_dir = fixture_root / course_id
...
report_json = {
    ...
    "suggestions": [
        {"line": 3, "original": "x2 + y2", "suggestion": "x^2 + y^2"},
        {"line": 5, "original": "int_0^1", "suggestion": "\\int_0^1"},
        {"line": 8, "original": "lim x->0", "suggestion": "\\lim_{x\\to0}"},
    ],
    "changed_lines": [3, 5],
    "fixture": "v0.7.5 owner-local smoke fixture",
}

report_md = """# OCR Math smoke fixture

This is a temporary owner-local diagnostic report used only by v0.7.5 smoke validation.
...
(course_dir / "ocr_math_report.json").write_text(json.dumps(report_json, ensure_ascii=False, indent=2), encoding="utf-8")
(course_dir / "ocr_math_report.md").write_text(report_md, encoding="utf-8")
...
for forbidden in ("Accept suggestion", "Reject suggestion", "Apply correction", "Auto-correct", "Formula OCR enabled"):
    if forbidden in viewer_text:
        raise SystemExit(f"viewer unexpectedly contains interactive/correction marker: {forbidden}")

missing_response = client.get("/owner/ocr-math-report/v0-7-5-missing-report/view")
if missing_response.status_code != 404:
    raise SystemExit(f"missing viewer should return 404, got {missing_response.status_code}")

print(json.dumps({
    "milestone": "v0.7.5-owner-local-ocr-math-report-syntax-and-smoke-fix-no-build-no-distribution",
    "status": "PASS",
...
$ForbiddenMilestoneArtifacts = Get-ChildItem $RepoRoot -Recurse -File -ErrorAction SilentlyContinue |
  Where-Object {
    ...
    $_.Name -match "v0\.7\.5.*\.(zip|msi|exe)$"
  }

if ($ForbiddenMilestoneArtifacts) {
  throw "Forbidden v0.7.5 build/distribution artifacts found: $($ForbiddenMilestoneArtifacts.FullName -join ', ')"
}

$AllowedChangedPaths = @(
  "services/api/web_app.py",
  "scripts/dev/check-v0.7.4-owner-local-ocr-math-report-smoke-fixture-no-build-no-distribution.ps1",
  "docs/dev/v0.7.5-owner-local-ocr-math-report-syntax-and-smoke-fix-no-build-no-distribution.md",
  "scripts/dev/check-v0.7.5-owner-local-ocr-math-report-syntax-and-smoke-fix-no-build-no-distribution.ps1"
)
...
foreach ($line in $Changed) {
  ...
  if ($AllowedChangedPaths -notcontains $normalized) {
    throw "Unexpected changed path for v0.7.5 repair milestone: $normalized"
  }
}
...
$Result = [ordered]@{
  ...
  repaired_web_app_syntax = $true
  smoke_passed = $true
  summary_route_preserved = $true
  raw_markdown_route_preserved = $true
  viewer_route_preserved = $true
