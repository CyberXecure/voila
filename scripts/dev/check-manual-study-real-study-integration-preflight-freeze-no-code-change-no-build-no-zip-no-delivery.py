...
doc = root / "docs" / "dev" / "manual-study-real-study-integration-preflight-freeze-no-code-change-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0821_WEB_APP_MISSING"),
    (doc, "FAILED_V0821_DOC_MISSING"),
]:
    if not path.exists():
...
for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0821_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Manual Study real `/study` integration preflight freeze",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0821_DOC_TERM_MISSING={term}")
...
allowed = {
    "docs/dev/manual-study-real-study-integration-preflight-freeze-no-code-change-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-manual-study-real-study-integration-preflight-freeze-no-code-change-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-manual-study-real-study-integration-preflight-freeze-no-code-change-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0821_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))

if web_text.count('@app.get("/owner/manual-study-integration-dry-run/{course_id}"') != 1:
    raise SystemExit("FAILED_V0821_DRY_RUN_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/owner/manual-study-integration-dry-run') != 0:
    raise SystemExit("FAILED_V0821_DRY_RUN_POST_ADDED")

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0821_STUDY_ROUTE_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0821_FORBIDDEN_WEB_TERM_FOUND={forbidden}")
...
if not study_route_match:
    raise SystemExit("FAILED_V0821_STUDY_ROUTE_STATIC_BLOCK_NOT_FOUND")
...
dry_run_route_match = re.search(
    r'@app\.get\("/owner/manual-study-integration-dry-run/\{course_id\}"[\s\S]*?(?=\n# VOILA_V0_8_18_MANUAL_STUDY_INTEGRATION_DRY_RUN_TOGGLE_END|\Z)',
    web_text,
)
if not dry_run_route_match:
    raise SystemExit("FAILED_V0821_DRY_RUN_ROUTE_STATIC_BLOCK_NOT_FOUND")

dry_run_route_block = dry_run_route_match.group(0)
dry_run_route_hash = hashlib.sha256(dry_run_route_block.encode("utf-8", errors="replace")).hexdigest()
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0821_POST={url}; STATUS={exc.code}; BODY={error_body}")
...
def fetch_url(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0821_{label}_FETCH={last_error}")

eligible_id = "v0821-check-preflight-freeze-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[212, 234, 600, 530]",
    "kind": "formula",
    "title": "v0.8.21 preflight freeze eligible",
    "verified_text": "Preflight freeze prompt text",
    "explanation_ro": "Răspuns read-only pentru preflight freeze.",
    "source_status": "verified",
    "source_note": "Created by v0.8.21 preflight freeze check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0821_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0821_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0821_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0821_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0821_MANUAL_STUDY_PREVIEW_JSON_MISSING")
...
if home_status != 200:
    raise SystemExit(f"FAILED_V0821_HOME_STATUS={home_status}")
if course_tools_status != 200:
    raise SystemExit(f"FAILED_V0821_COURSE_TOOLS_STATUS={course_tools_status}")
if dry_run_off_status != 200:
    raise SystemExit(f"FAILED_V0821_DRY_RUN_OFF_STATUS={dry_run_off_status}")
if dry_run_on_status != 200:
    raise SystemExit(f"FAILED_V0821_DRY_RUN_ON_STATUS={dry_run_on_status}")
...
for term in course_tools_terms:
    if term not in course_tools_body:
        raise SystemExit(f"FAILED_V0821_COURSE_TOOLS_TERM_MISSING={term}")
...
for term in off_terms:
    if term not in dry_run_off_body:
        raise SystemExit(f"FAILED_V0821_DRY_RUN_OFF_TERM_MISSING={term}")

if "Preflight freeze prompt text" in dry_run_off_body:
    raise SystemExit("FAILED_V0821_TOGGLE_OFF_RENDERED_CARD_TEXT")
...
on_terms = [
    ...
    "Preflight freeze prompt text",
    "Răspuns read-only pentru preflight freeze.",
    "manual_study_connected_to_real_study",
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0821_LEGACY_STUDY_PREVIEW_CHANGED")

manual_result = {
    "manual_preflight_status": "OWNER_LOCAL_MANUAL_STUDY_REAL_STUDY_INTEGRATION_PREFLIGHT_FREEZE_PASS",
    "static": {
        "study_route_block_sha256": study_route_hash,
        "dry_run_route_block_sha256": dry_run_route_hash,
        "study_route_present": True,
...
summary = {
    "VOILA_V0_8_21_MANUAL_STUDY_REAL_STUDY_INTEGRATION_PREFLIGHT_FREEZE_CHECK": "PASS",
    "depends_on_v0817_readiness_contract": True,
    "depends_on_v0818_dry_run_toggle": True,
    "depends_on_v0819_course_tools_link": True,
    "depends_on_v0820_browser_readiness_audit": True,
    "preflight_freeze_doc_added": True,
    "study_route_static_snapshot_recorded": True,
    "dry_run_route_static_snapshot_recorded": True,
    "study_route_block_sha256": study_route_hash,
    "dry_run_route_block_sha256": dry_run_route_hash,
    ...
    "v0822_change_plan_defined": True,
    ...
    "TESTER_READINESS": "BLOCKED_PREFLIGHT_ONLY_NO_REAL_STUDY_INTEGRATION_NO_PACKAGE",
    "POLICY": "manual_study_real_study_integration_preflight_freeze_no_code_change_no_build_no_zip_no_share_no_delivery_no_distribution",
    "RECOMMENDED_NEXT": "v0.8.22-owner-local-manual-study-real-study-read-only-shadow-toggle-no-progress-no-build-no-zip-no-delivery",
}

evidence = Path(r"D:\dev\tester-runs\v0821-manual-study-real-study-integration-preflight-freeze")
evidence.mkdir(parents=True, exist_ok=True)

out_json = evidence / "V0.8.21-MANUAL-STUDY-REAL-STUDY-INTEGRATION-PREFLIGHT-FREEZE-CHECK.json"
out_md = evidence / "V0.8.21-MANUAL-STUDY-REAL-STUDY-INTEGRATION-PREFLIGHT-FREEZE-CHECK.md"
...
md_lines.extend(["", "## Runtime steps"])
for step in manual_result["runtime_steps"]:
