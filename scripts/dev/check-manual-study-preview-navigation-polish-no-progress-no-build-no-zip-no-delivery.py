...
doc = root / "docs" / "dev" / "manual-study-preview-navigation-polish-no-progress-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0815_WEB_APP_MISSING"),
    (doc, "FAILED_V0815_DOC_MISSING"),
]:
    if not path.exists():
...
required_web_terms = [
    ...
    'data-testid="manual-study-preview-nav-jump"',
    'data-testid="manual-study-preview-position"',
    ...
    "manual study navigation polish",
]

for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0815_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Manual Study Preview navigation polish",
    "`/owner/manual-study-preview/{course_id}`",
    "top card jump links",
    "Previous link",
    "Next link",
    "Back to top link",
    "read-only in a `<details>` block",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0815_DOC_TERM_MISSING={term}")

if web_text.count('@app.get("/owner/manual-study-preview/{course_id}"') != 1:
    raise SystemExit("FAILED_V0815_MANUAL_STUDY_PREVIEW_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/owner/manual-study-preview') != 0:
    raise SystemExit("FAILED_V0815_MANUAL_STUDY_PREVIEW_POST_ADDED")

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0815_STUDY_ROUTE_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0815_FORBIDDEN_TERM_FOUND={forbidden}")
...
allowed = {
    ...
    "docs/dev/manual-study-preview-navigation-polish-no-progress-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-manual-study-preview-navigation-polish-no-progress-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-manual-study-preview-navigation-polish-no-progress-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0815_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0815_POST={url}; STATUS={exc.code}; BODY={error_body}")

eligible_id = "v0815-check-navigation-polish-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[162, 184, 500, 430]",
    "kind": "formula",
    "title": "v0.8.15 navigation polish eligible",
    "verified_text": "Navigation polish prompt text",
    "explanation_ro": "Răspuns afișat read-only cu navigare între carduri.",
    "source_status": "verified",
    "source_note": "Created by v0.8.15 navigation polish check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0815_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0815_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0815_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0815_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0815_MANUAL_STUDY_PREVIEW_JSON_MISSING")
...
def fetch_url(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0815_{label}_FETCH={last_error}")

preview_status, preview_body = fetch_url(manual_study_preview_url, "MANUAL_STUDY_PREVIEW")

if preview_status != 200:
    raise SystemExit(f"FAILED_V0815_PREVIEW_STATUS={preview_status}")
...
runtime_terms = [
    ...
    "manual-study-card-1",
    "Navigation polish prompt text",
    "Răspuns afișat read-only cu navigare între carduri.",
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0815_LEGACY_STUDY_PREVIEW_CHANGED")

summary = {
    "VOILA_V0_8_15_MANUAL_STUDY_PREVIEW_NAVIGATION_POLISH_CHECK": "PASS",
    ...
    "TESTER_READINESS": "BLOCKED_NAVIGATION_POLISH_ONLY_NO_PROGRESS",
    "POLICY": "manual_study_preview_navigation_polish_no_progress_no_build_no_zip_no_share_no_delivery_no_distribution",
}

evidence = Path(r"D:\dev\tester-runs\v0815-manual-study-preview-navigation-polish")
evidence.mkdir(parents=True, exist_ok=True)
out = evidence / "V0.8.15-MANUAL-STUDY-PREVIEW-NAVIGATION-POLISH-CHECK.json"
