...
doc = root / "docs" / "dev" / "manual-study-default-study-browser-readiness-audit-no-progress-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0828_WEB_APP_MISSING"),
    (doc, "FAILED_V0828_DOC_MISSING"),
]:
    if not path.exists():
...
for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0828_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Manual Study default Study browser readiness audit",
    ...
    "Default Manual Study has owner-local browser readiness evidence",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0828_DOC_TERM_MISSING={term}")
...
allowed = {
    "docs/dev/manual-study-default-study-browser-readiness-audit-no-progress-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-manual-study-default-study-browser-readiness-audit-no-progress-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-manual-study-default-study-browser-readiness-audit-no-progress-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0828_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0828_STUDY_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/study"') != 0:
    raise SystemExit("FAILED_V0828_STUDY_POST_ADDED")

if web_text.count("VOILA_V0_8_27_MANUAL_STUDY_DEFAULT_STUDY_READ_ONLY_FALLBACK_START") != 1:
    raise SystemExit("FAILED_V0828_DEFAULT_BLOCK_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0828_FORBIDDEN_WEB_TERM_FOUND={forbidden}")
...
if not study_route_match:
    raise SystemExit("FAILED_V0828_STUDY_ROUTE_STATIC_BLOCK_NOT_FOUND")
...
if study_route_hash != expected_v0821_study_route_hash:
    raise SystemExit(
        "FAILED_V0828_STUDY_ROUTE_HASH_CHANGED="
        + study_route_hash
        + ";EXPECTED="
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0828_POST={url}; STATUS={exc.code}; BODY={error_body}")
...
def fetch_url_allow_http_error(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0828_{label}_FETCH={last_error}")
...
eligible_id = "v0828-check-default-study-browser-readiness-audit-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[272, 294, 720, 650]",
    "kind": "formula",
    "title": "v0.8.28 default Study browser readiness eligible",
    "verified_text": "Default Study browser readiness prompt text",
    "explanation_ro": "Răspuns read-only pentru auditul browser al Study implicit.",
    "source_status": "verified",
    "source_note": "Created by v0.8.28 default Study browser readiness audit check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0828_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0828_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0828_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0828_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0828_MANUAL_STUDY_PREVIEW_JSON_MISSING")

home_status, home_body, _ = fetch_url_allow_http_error(home_url, "HOME")
if home_status != 200:
    raise SystemExit(f"FAILED_V0828_HOME_STATUS={home_status}; BODY={home_body[:800]}")
...
if course_tools_status != 200:
    raise SystemExit(f"FAILED_V0828_COURSE_TOOLS_STATUS={course_tools_status}; BODY={course_tools_body[:800]}")

if default_status != 200:
    raise SystemExit(f"FAILED_V0828_DEFAULT_STUDY_STATUS={default_status}; BODY={default_body[:800]}")

if shadow_status != 200:
    raise SystemExit(f"FAILED_V0828_SHADOW_STUDY_STATUS={shadow_status}; BODY={shadow_body[:800]}")
...
default_terms = [
    ...
    "Default Study browser readiness prompt text",
    "Răspuns read-only pentru auditul browser al Study implicit.",
]

for term in default_terms:
    if term not in default_body:
        raise SystemExit(f"FAILED_V0828_DEFAULT_STUDY_TERM_MISSING={term}")
...
for term in shadow_terms:
    if term not in shadow_body:
        raise SystemExit(f"FAILED_V0828_SHADOW_STUDY_TERM_MISSING={term}")
...
for term in course_tools_terms:
    if term not in course_tools_body:
        raise SystemExit(f"FAILED_V0828_COURSE_TOOLS_TERM_MISSING={term}")

manual_study = json.loads(manual_study_preview.read_text(encoding="utf-8", errors="replace"))
items = manual_study.get("items")
if not isinstance(items, list) or not items:
    raise SystemExit("FAILED_V0828_ITEMS_INVALID_OR_EMPTY")

ids = {item.get("source_evidence_id") for item in items if isinstance(item, dict)}
if eligible_id not in ids:
    raise SystemExit("FAILED_V0828_ELIGIBLE_ID_NOT_EXPORTED")

fallback_status = None
fallback_body = ""
tmp_preview = manual_study_preview.with_name(manual_study_preview.name + ".v0828tmp")
...
if fallback_status != 200:
    raise SystemExit(f"FAILED_V0828_LEGACY_FALLBACK_STATUS={fallback_status}; BODY={fallback_body[:800]}")

if "manual-study-default-route" in fallback_body:
    raise SystemExit("FAILED_V0828_LEGACY_FALLBACK_RENDERED_MANUAL_DEFAULT_WHEN_PREVIEW_MISSING")
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0828_LEGACY_STUDY_PREVIEW_CHANGED")

manual_result = {
    "manual_browser_audit_status": "OWNER_LOCAL_DEFAULT_STUDY_MANUAL_STUDY_BROWSER_READINESS_PASS",
    "steps": [
        {"step": "course_tools_loads", "status": "PASS", "http_status": course_tools_status},
        {"step": "course_tools_shadow_link_still_visible", "status": "PASS"},
        {"step": "course_tools_dry_run_link_still_visible", "status": "PASS"},
        {"step": "default_study_route_loads", "status": "PASS", "http_status": default_status},
        {"step": "default_study_renders_manual_study_cards", "status": "PASS"},
        ...
        {"step": "legacy_fallback_when_manual_preview_missing", "status": "PASS", "http_status": fallback_status},
        {"step": "legacy_study_items_preview_unchanged", "status": "PASS"},
    ],
}

summary = {
    "VOILA_V0_8_28_MANUAL_STUDY_DEFAULT_STUDY_BROWSER_READINESS_AUDIT_CHECK": "PASS",
    "depends_on_v0827_default_study_read_only_fallback": True,
    ...
    "manual_study_default_visible": True,
    ...
    "manual_study_connected_to_real_study": "default_read_only_with_legacy_fallback_existing_v0827",
    ...
    "TESTER_READINESS": "BLOCKED_BROWSER_AUDIT_ONLY_NO_PROGRESS_NO_PACKAGE",
    "POLICY": "manual_study_default_study_browser_readiness_audit_no_progress_no_build_no_zip_no_share_no_delivery_no_distribution",
}

evidence = Path(r"D:\dev\tester-runs\v0828-manual-study-default-study-browser-readiness-audit")
evidence.mkdir(parents=True, exist_ok=True)

out_json = evidence / "V0.8.28-MANUAL-STUDY-DEFAULT-STUDY-BROWSER-READINESS-AUDIT-CHECK.json"
out_md = evidence / "V0.8.28-MANUAL-STUDY-DEFAULT-STUDY-BROWSER-READINESS-AUDIT-CHECK.md"
