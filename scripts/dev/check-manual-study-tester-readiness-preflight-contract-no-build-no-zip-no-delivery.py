...
doc = root / "docs" / "dev" / "manual-study-tester-readiness-preflight-contract-no-build-no-zip-no-delivery.md"

for path, marker in [
    (web, "FAILED_V0831_WEB_APP_MISSING"),
    (doc, "FAILED_V0831_DOC_MISSING"),
]:
    if not path.exists():
...
for term in required_web_terms:
    if term not in web_text:
        raise SystemExit(f"FAILED_V0831_WEB_TERM_MISSING={term}")

required_doc_terms = [
    "Manual Study tester readiness preflight contract",
    "before any tester readiness claim, package rebuild, ZIP creation, share, or delivery",
    ...
    "Upload or existing generated course opens in the tester UI path.",
    "Course Tools is reachable from the normal tester navigation.",
    "Normal Study link from Course Tools opens Manual Study default.",
    "Manual Study default shows readable cards.",
    "No new Study POST endpoint is introduced.",
...
for term in required_doc_terms:
    if term not in doc_text:
        raise SystemExit(f"FAILED_V0831_DOC_TERM_MISSING={term}")
...
allowed = {
    "docs/dev/manual-study-tester-readiness-preflight-contract-no-build-no-zip-no-delivery.md",
    "scripts/dev/check-manual-study-tester-readiness-preflight-contract-no-build-no-zip-no-delivery.py",
    "scripts/dev/check-manual-study-tester-readiness-preflight-contract-no-build-no-zip-no-delivery.ps1",
}

unexpected = sorted(path for path in changed if path and path.replace("\\", "/") not in allowed)
if unexpected:
    raise SystemExit("FAILED_V0831_UNEXPECTED_CHANGED_FILES=" + ",".join(unexpected))

if web_text.count('@app.get("/study"') != 1:
    raise SystemExit("FAILED_V0831_STUDY_ROUTE_COUNT_CHANGED")

if web_text.count('@app.post("/study"') != 0:
    raise SystemExit("FAILED_V0831_STUDY_POST_ADDED")

if web_text.count("VOILA_V0_8_27_MANUAL_STUDY_DEFAULT_STUDY_READ_ONLY_FALLBACK_START") != 1:
    raise SystemExit("FAILED_V0831_DEFAULT_BLOCK_COUNT_CHANGED")
...
for forbidden in [
    ...
    if forbidden in web_text:
        raise SystemExit(f"FAILED_V0831_FORBIDDEN_WEB_TERM_FOUND={forbidden}")
...
if not study_route_match:
    raise SystemExit("FAILED_V0831_STUDY_ROUTE_STATIC_BLOCK_NOT_FOUND")
...
if study_route_hash != expected_v0821_study_route_hash:
    raise SystemExit(
        "FAILED_V0831_STUDY_ROUTE_HASH_CHANGED="
        + study_route_hash
        + ";EXPECTED="
...
def post_form(url, payload):
    ...
    try:
        ...
        raise SystemExit(f"FAILED_V0831_POST={url}; STATUS={exc.code}; BODY={error_body}")
...
def fetch_url_allow_http_error(url, label):
    ...
    for _ in range(10):
        try:
            ...
            last_error = str(exc)
            time.sleep(2)
    raise SystemExit(f"FAILED_V0831_{label}_FETCH={last_error}")
...
eligible_id = "v0831-check-tester-readiness-preflight-contract-eligible"

payload = {
    "draft_id": eligible_id,
    "page": "1",
    "bbox": "[302, 324, 780, 710]",
    "kind": "formula",
    "title": "v0.8.31 tester readiness preflight eligible",
    "verified_text": "Tester readiness preflight prompt text",
    "explanation_ro": "Răspuns read-only pentru contractul preflight tester readiness.",
    "source_status": "verified",
    "source_note": "Created by v0.8.31 tester readiness preflight contract check.",
}

status, body, _ = post_form(save_url, payload)
if status != 200:
    raise SystemExit(f"FAILED_V0831_SAVE_FIXTURE_STATUS={status}; BODY={body}")

verify_status, verify_body, _ = post_form(verify_url, {"draft_id": eligible_id})
if verify_status != 200:
    raise SystemExit(f"FAILED_V0831_VERIFY_FIXTURE_STATUS={verify_status}; BODY={verify_body}")

learning_export_status, learning_export_body, _ = post_form(learning_pack_export_url, {})
if learning_export_status != 200:
    raise SystemExit(f"FAILED_V0831_LEARNING_PACK_EXPORT_STATUS={learning_export_status}; BODY={learning_export_body}")

manual_study_export_status, manual_study_export_body, _ = post_form(manual_study_export_url, {})
if manual_study_export_status != 200:
    raise SystemExit(f"FAILED_V0831_MANUAL_STUDY_EXPORT_STATUS={manual_study_export_status}; BODY={manual_study_export_body}")

if not manual_study_preview.exists():
    raise SystemExit("FAILED_V0831_MANUAL_STUDY_PREVIEW_JSON_MISSING")

home_status, home_body, _ = fetch_url_allow_http_error(home_url, "HOME")
if home_status != 200:
    raise SystemExit(f"FAILED_V0831_HOME_STATUS={home_status}; BODY={home_body[:800]}")
...
if course_tools_status != 200:
    raise SystemExit(f"FAILED_V0831_COURSE_TOOLS_STATUS={course_tools_status}; BODY={course_tools_body[:800]}")
...
if normal_study_href is None:
    raise SystemExit("FAILED_V0831_NORMAL_STUDY_LINK_HREF_NOT_FOUND_OR_UNEXPECTED=" + ",".join(normal_study_hrefs[:10]))
...
if not shadow_link_match:
    raise SystemExit("FAILED_V0831_SHADOW_LINK_HREF_NOT_FOUND")
...
if not dry_run_link_match:
    raise SystemExit("FAILED_V0831_DRY_RUN_LINK_HREF_NOT_FOUND")
...
if normal_status != 200:
    raise SystemExit(f"FAILED_V0831_NORMAL_STUDY_STATUS={normal_status}; BODY={normal_body[:800]}")

if shadow_status != 200:
    raise SystemExit(f"FAILED_V0831_SHADOW_STATUS={shadow_status}; BODY={shadow_body[:800]}")

if dry_run_status != 200:
    raise SystemExit(f"FAILED_V0831_DRY_RUN_STATUS={dry_run_status}; BODY={dry_run_body[:800]}")
...
normal_terms = [
    ...
    "Tester readiness preflight prompt text",
    "Răspuns read-only pentru contractul preflight tester readiness.",
]

for term in normal_terms:
    if term not in normal_body:
        raise SystemExit(f"FAILED_V0831_NORMAL_STUDY_TERM_MISSING={term}")
...
for term in shadow_terms:
    if term not in shadow_body:
        raise SystemExit(f"FAILED_V0831_SHADOW_TERM_MISSING={term}")
...
for term in dry_run_terms:
    if term not in dry_run_body:
        raise SystemExit(f"FAILED_V0831_DRY_RUN_TERM_MISSING={term}")

manual_study = json.loads(manual_study_preview.read_text(encoding="utf-8", errors="replace"))
items = manual_study.get("items")
if not isinstance(items, list) or not items:
    raise SystemExit("FAILED_V0831_ITEMS_INVALID_OR_EMPTY")

ids = {item.get("source_evidence_id") for item in items if isinstance(item, dict)}
if eligible_id not in ids:
    raise SystemExit("FAILED_V0831_ELIGIBLE_ID_NOT_EXPORTED")

fallback_status = None
fallback_body = ""
tmp_preview = manual_study_preview.with_name(manual_study_preview.name + ".v0831tmp")
...
if fallback_status != 200:
    raise SystemExit(f"FAILED_V0831_NORMAL_STUDY_LEGACY_FALLBACK_STATUS={fallback_status}; BODY={fallback_body[:800]}")

if "manual-study-default-route" in fallback_body:
    raise SystemExit("FAILED_V0831_NORMAL_STUDY_FALLBACK_RENDERED_MANUAL_DEFAULT_WHEN_PREVIEW_MISSING")
...
if legacy_before != legacy_after:
    raise SystemExit("FAILED_V0831_LEGACY_STUDY_PREVIEW_CHANGED")

summary = {
    "VOILA_V0_8_31_MANUAL_STUDY_TESTER_READINESS_PREFLIGHT_CONTRACT_CHECK": "PASS",
    "depends_on_v0830_full_manual_study_flow_readiness_audit": True,
    "preflight_contract_doc_added": True,
    "owner_local_manual_study_flow_ready": True,
    ...
    "shadow_route_still_available": True,
    "shadow_route_status": shadow_status,
    "dry_run_route_still_available": True,
    ...
    "tester_readiness_contract_defined": True,
    "package_readiness_blockers_defined": True,
    "requires_separate_tester_readiness_milestone": True,
    "requires_separate_package_rebuild_milestone": True,
    "requires_separate_extracted_package_validation": True,
    "requires_explicit_owner_approval_before_delivery": True,
    "web_app_changed": False,
    "new_route_added": False,
    "new_post_endpoint_added": False,
    "manual_study_connected_to_real_study": "owner_local_ready_tester_readiness_preflight_only",
    ...
    "TESTER_READINESS": "BLOCKED_PREFLIGHT_ONLY_NO_PACKAGE_NO_EXTRACTED_VALIDATION_NO_DELIVERY",
    "POLICY": "manual_study_tester_readiness_preflight_contract_no_build_no_zip_no_share_no_delivery_no_distribution",
    "RECOMMENDED_NEXT": "v0.8.32-owner-local-manual-study-full-tester-flow-readiness-audit-no-build-no-zip-no-delivery",
}

evidence = Path(r"D:\dev\tester-runs\v0831-manual-study-tester-readiness-preflight-contract")
evidence.mkdir(parents=True, exist_ok=True)

out_json = evidence / "V0.8.31-MANUAL-STUDY-TESTER-READINESS-PREFLIGHT-CONTRACT-CHECK.json"
out_md = evidence / "V0.8.31-MANUAL-STUDY-TESTER-READINESS-PREFLIGHT-CONTRACT-CHECK.md"
