...
Write-Host "=== v0.6.25 controlled tester package release readiness freeze no-build no-distribution check ==="

$repoRoot = (git rev-parse --show-toplevel).Trim()
Set-Location $repoRoot

$docPath = "docs/dev/controlled-tester-package-release-readiness-freeze-no-build-no-distribution.md"
$checkPath = "scripts/dev/check-controlled-tester-package-release-readiness-freeze-no-build-no-distribution.ps1"
$allowedTouched = @($docPath, $checkPath)

$freezeRoot = "D:\dev\release-assets\voila\v0.6.25-controlled-tester-package-release-readiness-freeze-no-build-no-distribution"
$freezePath = Join-Path $freezeRoot "RELEASE-READINESS-FREEZE-DRY-RUN.json"
$readmePath = Join-Path $freezeRoot "RELEASE-READINESS-FREEZE-README.txt"
...
foreach ($path in $allowedTouched) {
  Assert-True (Test-Path -LiteralPath $path) "Missing required v0.6.25 file: $path"
  Write-Host $path
}
...
$v0624DocText = Get-Content -LiteralPath $previous.v0624Doc -Raw
...
Assert-True ($v0624DocText -match "v0\.6\.25-controlled-tester-package-release-readiness-freeze-no-build-no-distribution") "v0.6.24 allowed next marker missing."

Assert-True ($v0624Text -match "release_readiness_review_created.*true") "v0.6.24 must have release readiness review."
Assert-True ($v0624Text -match "release_readiness_review_checked.*true") "v0.6.24 must check release readiness review."
Assert-True ($v0624Text -match "release_readiness_review_dry_run_only.*true") "v0.6.24 must keep release readiness review dry-run only."
Assert-True ($v0624Text -match "release_readiness_approved_now.*false") "v0.6.24 must keep release readiness approval blocked."
Assert-True ($v0624Text -match "release_approval_allowed_now.*false") "v0.6.24 must keep release approval blocked."
Assert-True ($v0624Text -match "release_approval_unfreeze_allowed_now.*false") "v0.6.24 must keep release approval unfreeze blocked."
Assert-True ($v0624Text -match "public_release_allowed_now.*false") "v0.6.24 must keep public release blocked."
Assert-True ($v0624Text -match "tester_delivery_allowed_now.*false") "v0.6.24 must keep tester delivery blocked."
Assert-True ($v0624Text -match "distribution_allowed.*false") "v0.6.24 must keep distribution blocked."
Assert-True ($v0624Text -match "package_build_executed_now.*false") "v0.6.24 must keep package build blocked."
Assert-True ($v0624Text -match "zip_created.*false") "v0.6.24 must keep zip creation blocked."
Assert-True ($v0624Text -match "release_archive_created.*false") "v0.6.24 must keep release archive blocked."
...
foreach ($path in $touchedPaths) {
  Write-Host $path
  Assert-True ($allowedTouched -contains $path) "Unexpected changed path in v0.6.25 milestone: $path"
}
...
foreach ($path in $touchedPaths) {
  foreach ($pattern in $forbiddenPathPatterns) {
    Assert-True (-not ($path -match $pattern)) "Forbidden path touched by v0.6.25 release-readiness-freeze milestone: $path"
  }
}
...
$requiredDocMarkers = @(
  ...
  "No live scoring",
  "release_readiness_freeze_created = true",
  "release_readiness_freeze_active = true",
  "release_readiness_frozen = true",
  ...
  "This milestone records release readiness freeze only"
)
foreach ($marker in $requiredDocMarkers) {
  Assert-True ($docText -match [regex]::Escape($marker)) "Doc missing safety marker: $marker"
}

Assert-True ($checkText -match "release_readiness_freeze_created.*true") "Check must set release_readiness_freeze_created true."
Assert-True ($checkText -match "release_readiness_freeze_active.*true") "Check must set release_readiness_freeze_active true."
Assert-True ($checkText -match "release_readiness_frozen.*true") "Check must set release_readiness_frozen true."
Assert-True ($checkText -match "release_readiness_review_checked.*true") "Check must keep release_readiness_review_checked true."
...
Write-Host ""
Write-Host "=== Create local-only release readiness freeze evidence directory ==="
...
Assert-True (-not ($freezeFull.StartsWith($repoFull, [System.StringComparison]::OrdinalIgnoreCase))) "Release readiness freeze root must be outside repository."
...
$freeze = [ordered]@{
  schema_version = "1"
  release_readiness_freeze_version = "v0.6.25"
  release_readiness_freeze_type = "controlled_tester_package_release_readiness_freeze_no_build_no_distribution"
  status = "pass"
  release_readiness_freeze = [ordered]@{
    owner_controlled_release_readiness_freeze_created = $true
    release_readiness_freeze_created = $true
    release_readiness_freeze_active = $true
    release_readiness_frozen = $true
    release_readiness_review_checked = $true
    ...
    next_step_policy = "STOP_OR_SEPARATE_DISTRIBUTION_READINESS_PLAN_NO_BUILD_MILESTONE_ONLY"
  }
  frozen_release_readiness_categories = $frozenEntries
  evidence = [ordered]@{
    previous_release_readiness_review_version = "v0.6.24"
    previous_release_readiness_review_check = $previous.v0624Check
  ...
  gates = [ordered]@{
    ...
    release_readiness_freeze_only = $true
    freeze_root_outside_repo = $true
    release_readiness_freeze_active = $true
    release_readiness_frozen = $true
    release_readiness_review_checked = $true
  ...
  safety = [ordered]@{
    ...
    creates_local_release_readiness_freeze_evidence = $true
    freezes_release_readiness = $true
  ...
  allowed_next_milestones = @("STOP", "v0.6.26-controlled-tester-package-distribution-readiness-plan-no-build-no-distribution")
...
$readme = @"
Voila v0.6.25 controlled tester package release readiness freeze no-build no-distribution
...
$allowedFreezeNames = @("RELEASE-READINESS-FREEZE-DRY-RUN.json", "RELEASE-READINESS-FREEZE-README.txt")
foreach ($name in $freezeFiles) {
  Assert-True ($allowedFreezeNames -contains $name) "Unexpected file in local-only release readiness freeze evidence directory: $name"
  Assert-True (-not ($name -match "\.(zip|exe|msi)$")) "Forbidden package/release artifact in release readiness freeze evidence directory: $name"
}

Assert-True (Test-Path -LiteralPath $freezePath) "Missing release-readiness-freeze JSON."
Assert-True (Test-Path -LiteralPath $readmePath) "Missing release-readiness-freeze README."

Write-Host ""
Write-Host "=== v0.6.25 release readiness freeze JSON ==="
$freezeJson
...
Write-Host ""
Write-Host "=== v0.6.25 CONTROLLED TESTER PACKAGE RELEASE READINESS FREEZE NO-BUILD NO-DISTRIBUTION CHECK PASS ==="
