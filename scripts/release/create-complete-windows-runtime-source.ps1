...
function Write-Launchers {
  ...
  Write-TextFile -Path (Join-Path $scriptsDir "start-voila.ps1") -Lines @(
    ...
    '$proc = Start-Process -FilePath $PythonExe -ArgumentList $args -WorkingDirectory $PackageRoot -WindowStyle Hidden -PassThru',
    '$proc.Id | Set-Content -Encoding UTF8 $apiPid',
    '"Voila API process launch requested. PID: $($proc.Id)" | Add-Content -Encoding UTF8 $startLog',
    '',
    ...
    '    Write-Host "Voila API exited early with code $($proc.ExitCode)"',
    '    exit 2',
    '  }',
    '',
    '  if ((Test-UrlResponds -Url "http://127.0.0.1:8787/health") -or (Test-UrlResponds -Url "http://127.0.0.1:8787")) {',
    '    $started = $true',
    '    break',
    '  }',
    '',
    ...
    '}',
    '',
    '"Voila API started. PID: $($proc.Id)" | Add-Content -Encoding UTF8 $startLog',
