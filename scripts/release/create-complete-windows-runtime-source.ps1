...
function Write-Launchers {
  ...
  Write-TextFile -Path (Join-Path $scriptsDir "start-voila.ps1") -Lines @(
    ...
    '  "Package-local Python not found: $PythonExe" | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "Package-local Python not found: $PythonExe"',
    '  exit 2',
    '}',
    '',
    'if (-not (Test-Path (Join-Path $ApiDir "web_app.py") -PathType Leaf)) {',
    '  "Voila API entrypoint not found: services\api\web_app.py" | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "Voila API entrypoint not found: services\api\web_app.py"',
    '  exit 2',
    '}',
    '',
    'if (Test-PortInUse -Port $Port) {',
    '  "Port 8787 is already in use before START." | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "Port 8787 is already in use before START."',
    '  exit 3',
    '}',
    '',
    '$runner = Join-Path $StateDir "run-voila-api.cmd"',
    '$runnerLines = @(',
    '  "@echo off",',
    '  (''cd /d "{0}"'' -f $PackageRoot),',
    '  (''"{0}" -B -m uvicorn web_app:app --app-dir .\services\api --host 127.0.0.1 --port 8787 >> "{1}" 2>> "{2}"'' -f $PythonExe, $stdoutLog, $stderrLog)',
    ')',
    'Set-Content -Encoding ASCII -Path $runner -Value $runnerLines',
    '$commandLine = ''cmd.exe /d /c "'' + $runner + ''"''',
    '"Voila API WMI launch command prepared: $commandLine" | Add-Content -Encoding UTF8 $startLog',
    'try {',
    ...
    '  }',
    '} catch {',
    '  "WMI launch failed: $($_.Exception.Message)" | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "WMI launch failed: $($_.Exception.Message)"',
    '  exit 2',
    '}',
    'if (-not $createResult -or $createResult.ReturnValue -ne 0) {',
    '  "WMI launch returned failure. ReturnValue=$($createResult.ReturnValue)" | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "WMI launch returned failure. ReturnValue=$($createResult.ReturnValue)"',
    '  exit 2',
    '}',
    '"Voila API WMI launch requested. Bootstrap PID: $($createResult.ProcessId)" | Add-Content -Encoding UTF8 $startLog',
    '',
    ...
    '',
    '  if ((Test-UrlResponds -Url "http://127.0.0.1:8787/health") -or (Test-UrlResponds -Url "http://127.0.0.1:8787") -or (Test-PortInUse -Port $Port)) {',
    '    $ownerPid = Get-PortOwnerPid -Port $Port',
    '    if ($ownerPid) {',
    '      $ownerPid | Set-Content -Encoding UTF8 $apiPid',
    '      $started = $true',
    '      break',
    '    }',
    '  }',
    ...
    '',
    'if (-not $started) {',
    '  "Voila API did not become reachable within startup timeout." | Add-Content -Encoding UTF8 $startLog',
    '  Write-Host "Voila API did not become reachable within startup timeout."',
    '  exit 4',
    '}',
    '',
    '"Voila API started. PID: $ownerPid" | Add-Content -Encoding UTF8 $startLog',
    'Write-Host "Voila API started. PID: $ownerPid"',
